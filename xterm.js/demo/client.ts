...
import {
    SearchAddon,
    ISearchOptions,
} from "../addons/addon-search/out/SearchAddon";
...
import type {
    ImageAddonType,
    IImageAddonOptions,
} from "../addons/addon-image/out/ImageAddon";
...
const addons: { [T in AddonType]: IDemoAddon<T> } = {
    ...
    unicodeGraphemes: {
        name: "unicodeGraphemes",
        ctor: UnicodeGraphemesAddon,
        canChange: true,
    },
...
if (document.location.pathname === "/test") {
    ...
    document
        ...
        .addEventListener("click", htmlSerializeButtonHandler);
    document
        .getElementById("custom-glyph")
        .addEventListener("click", writeCustomGlyphHandler);
    document.getElementById("load-test").addEventListener("click", loadTest);
    document.getElementById("print-cjk").addEventListener("click", addCjk);
    document
        .getElementById("print-cjk-sgr")
        .addEventListener("click", addCjkRandomSgr);
    document
        .getElementById("powerline-symbol-test")
        .addEventListener("click", powerlineSymbolTest);
    document
        .getElementById("underline-test")
        .addEventListener("click", underlineTest);
    document
        .getElementById("ansi-colors")
        .addEventListener("click", ansiColorsTest);
    document
        .getElementById("osc-hyperlinks")
        .addEventListener("click", addAnsiHyperlink);
    document.getElementById("sgr-test").addEventListener("click", sgrTest);
    document
        .getElementById("add-grapheme-clusters")
        .addEventListener("click", addGraphemeClusters);
    document
        .getElementById("add-decoration")
        .addEventListener("click", addDecoration);
    document
        .getElementById("add-overview-ruler")
        .addEventListener("click", addOverviewRuler);
    document
        .getElementById("weblinks-test")
        .addEventListener("click", testWeblinks);
...
function createTerminal(): void {
    ...
    if (addons.webgl.instance) {
        try {
            ...
            addons.webgl.instance.onChangeTextureAtlas((e) =>
                setTextureAtlas(e)
            );
            addons.webgl.instance.onAddTextureAtlasCanvas((e) =>
                appendTextureAtlas(e)
            );
            addons.webgl.instance.onRemoveTextureAtlasCanvas((e) =>
                removeTextureAtlas(e)
            );
        } catch (e) {
            ...
            if (checkbox.checked) {
                ...
                try {
                    ...
                    if (name === "webgl") {
                        ...
                        setTimeout(() => {
                            setTextureAtlas(
                                addons.canvas.instance.textureAtlas
                            );
                            addons.canvas.instance.onChangeTextureAtlas((e) =>
                                setTextureAtlas(e)
                            );
                            addons.canvas.instance.onAddTextureAtlasCanvas(
                                (e) => appendTextureAtlas(e)
                            );
                        }, 0);
                        ...
                        addon.instance.onDidChangeResults((e) =>
                            updateFindResults(e)
                        );
                    }
                } catch {
...
function updateTerminalSize(): void {
    const width = autoResize
        ? "100%"
        : (
              term._core._renderService.dimensions.css.canvas.width +
...
function serializeButtonHandler(): void {
    ...
    if (
        (document.getElementById("write-to-terminal") as HTMLInputElement)
            .checked
    ) {
        term.reset();
        term.write(output);
...
function writeCustomGlyphHandler(): void {
    ...
    term.write("╰ ╱ ╲ ╳ ╴ ╵ ╶ ╷ ╸ ╹ ╺ ╻ ╼ ╽ ╾ ╿\n\r");
    term.write(
        "Box drawing alignment tests:\x1b[31m                                          █\n\r"
    );
    term.write(
        "                                                                      ▉\n\r"
    );
    ...
    term.write(
        "Box drawing alignment tests:\x1b[32m                                          █\n\r"
    );
    term.write(
        "                                                                      ▉\n\r"
    );
...
function loadTest(): void {
    const rendererName = addons.webgl.instance
        ? "webgl"
        : !!addons.canvas.instance
          ? "canvas"
          : "dom";
    const testData = [];
    let byteCount = 0;
...
function powerlineSymbolTest(): void {
    ...
    term.writeln(`0xA_                 ${s("\ue0a3")}`);
    term.writeln(
        `0xB_                      ${s("\ue0b4")}${s("\ue0b5")}${s("\ue0b6")}${s("\ue0b7")}${s("\ue0b8")}${s("\ue0b9")}${s("\ue0ba")}${s("\ue0bb")}${s("\ue0bc")}${s("\ue0bd")}${s("\ue0be")}${s("\ue0bf")}`
    );
    term.writeln(
        `0xC_  ${s("\ue0c0")}${s("\ue0c1")}${s("\ue0c2")}${s("\ue0c3")}${s("\ue0c4")}${s("\ue0c5")}${s("\ue0c6")}${s("\ue0c7")}${s("\ue0c8")}${s("\ue0c9")}${s("\ue0ca")}${s("\ue0cb")}${s("\ue0cc")}${s("\ue0cd")}${s("\ue0be")}${s("\ue0bf")}`
    );
    term.writeln(
        `0xD_  ${s("\ue0d0")}${s("\ue0d1")}${s("\ue0d2")}     ${s("\ue0d4")}`
    );
...
function ansiColorsTest(): void {
    ...
    for (let i = 0; i < 16; i++) {
        term.write(
            `\x1b[48;5;${i}m ${i.toString().padEnd(2, " ").padStart(3, " ")} \x1b[0m`
        );
    }
    ...
    for (let i = 0; i < 6; i++) {
        ...
        term.write(
            `${startId.toString().padStart(3, " ")}-${endId.toString().padStart(3, " ")} `
        );
        for (let j = 0; j < 36; j++) {
            const id = 16 + i * 36 + j;
            term.write(
                `\x1b[48;5;${id}m${(id % 10).toString().padStart(2, " ")}\x1b[0m`
            );
        }
        term.write(`\r\n`);
    }
...
function sgrTest(): void {
    ...
    const maxNameLength = entries.reduce<number>(
        (p, c) => Math.max(c.name.length, p),
        0
    );
    for (const e of entries) {
        term.writeln(
            `\x1b[0m\x1b[${e.ps}m ${e.ps.toString().padEnd(2, " ")} ${e.name.padEnd(maxNameLength, " ")} - ${testString}\x1b[0m`
        );
    }
...
function addAnsiHyperlink(): void {
    ...
    term.writeln(
        "\x1b]8;;https://xtermjs.org\x07https://xtermjs.org\x1b]8;;\x07\x1b[C<- null cell"
    );
    term.writeln(`\nAdjacent links:`);
    term.writeln(
...
function getRandomSgr(): string {
    return randomSgrAttributes[
        Math.floor(Math.random() * randomSgrAttributes.length)
    ];
}
...
function addGraphemeClusters(): void {
    term.write("\n\n\r");
    term.writeln(
        "🤣🤣🤣🤣🤣🤣🤣🤣🤣🤣 [Simple emoji v6: 10 cells, v15: 20 cells]"
    );
    term.writeln(
        "\u{1F476}\u{1F3FF}\u{1F476} [baby with emoji modifier fitzpatrick type-6; baby]"
    );
    term.writeln(
        "\u{1F469}\u200d\u{1f469}\u200d\u{1f466} [woman+zwj+woman+zwj+boy]"
    );
    term.writeln(
        "\u{1F64B}\u{1F64B}\u{200D}\u{2642}\u{FE0F} [person/man raising hand]"
    );
    term.writeln(
    ...
    term.writeln(
        "\u{1f1f3}\u{1f1f4}_ [REGIONAL INDICATOR SYMBOL LETTER N and RI O]"
    );
...
function addVtButtons(): void {
    ...
    function createButton(
        ...
        element.addEventListener(`click`, () =>
            term.write(
                csi(`${prefix}${inputs.map((e) => e.value).join(";")}${suffix}`)
            )
        );
    ...
    const buttonSpecs: {
        ...
        H: {
            label: "CUP",
            description: "Cursor Position [row;column]",
            paramCount: 2,
        },
        I: {
            label: "CHT",
            description: "Cursor Forward Tabulation Ps tab stops",
        },
    ...
    for (const s of Object.keys(buttonSpecs)) {
        ...
        vtFragment.appendChild(
            createButton(spec.label, spec.description, s, spec.paramCount)
        );
    }
...
function initImageAddonExposed(): void {
    const DEFAULT_OPTIONS: IImageAddonOptions = (addons.image.instance as any)
        ._defaultOpts;
    const limitStorageElement = document.querySelector<HTMLInputElement>(
        "#image-storagelimit"
    );
    limitStorageElement.valueAsNumber = addons.image.instance.storageLimit;
    addDomListener(limitStorageElement, "change", () => {
        try {
            addons.image.instance.storageLimit =
                limitStorageElement.valueAsNumber;
            limitStorageElement.valueAsNumber =
                addons.image.instance.storageLimit;
            console.log(
                "changed storageLimit to",
                addons.image.instance.storageLimit
            );
        } catch (e) {
            limitStorageElement.valueAsNumber =
    ...
    const showPlaceholderElement = document.querySelector<HTMLInputElement>(
    ...
    const ctorOptionsElement =
        document.querySelector<HTMLTextAreaElement>("#image-options");
    ...
    const iipDemo = (url: string) => () =>
        fetch(url)
            ...
            .then((buffer) => {
                ...
                term.write("\r\n");
                term.write(
                    `\x1b]1337;File=inline=1;size=${data.length}:${btoa(sdata)}\x1b\\`
                );
            });
    ...
    document
        ...
        .addEventListener(
            "click",
            sixelDemo(
                "https://raw.githubusercontent.com/jerch/node-sixel/master/testfiles/test2.sixel"
            )
        );
    document
        ...
        .addEventListener(
            "click",
            iipDemo(
                "https://raw.githubusercontent.com/jerch/node-sixel/master/palette.png"
            )
        );
    ...
    term.element.addEventListener("click", (ev: MouseEvent) => {
        ...
        const canvas = ev.shiftKey
            ? // ctrl+shift+click: get single tile
              addons.image.instance.extractTileAtBufferCell(
                  x,
                  term.buffer.active.viewportY + y
              )
            : // ctrl+click: get original image
              addons.image.instance.getImageAtBufferCell(
                  x,
                  term.buffer.active.viewportY + y
              );
        canvas?.toBlob((data) =>
...
function testEvents(): void {
    ...
    document
        .getElementById("event-blur")
        .addEventListener("click", () => term.blur());
}
