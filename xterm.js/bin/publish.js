...
function checkAndPublishPackage(packageDir) {
    ...
    let nextVersion = isStableRelease
        ? packageJson.version
        : getNextBetaVersion(packageJson);
...
function getNextBetaVersion(packageJson) {
    ...
    const latestTagVersion = parseInt(
        latestPublishedVersion.slice(latestPublishedVersion.search(/\d+$/)),
        10
    );
...
function getPublishedVersions(packageJson, version, tag) {
    ...
    if (versionsProcess.stdout.length === 0 && versionsProcess.stderr) {
        ...
        if (
            err.indexOf(
                "404 Not Found - GET https://registry.npmjs.org/@xterm"
            ) > 0
        ) {
            return [];
        }
    ...
    if (tag) {
        return versionsJson.filter(
            (v) => !v.search(new RegExp(`${version}-${tag}.[0-9]+`))
        );
    }
    return versionsJson;
}
...
function updateWebsite() {
    ...
    if (!isDryRun) {
        cp.spawnSync(
            "sh",
            [path.join(__dirname, "update-website.sh"), packageJson.version],
            { cwd, stdio: [process.stdin, process.stdout, process.stderr] }
        );
    }
}
