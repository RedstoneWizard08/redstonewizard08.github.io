...
let testFiles = ["./out/**/*test.js", "./addons/**/out/*test.js"];
...
if (checkCoverage) {
    ...
    const run = cp.spawnSync(executable, args, {
...
const run = cp.spawnSync(npmBinScript("mocha"), [...testFiles, ...flagArgs], {
    cwd: path.resolve(__dirname, ".."),
    env,
    stdio: "inherit",
});
...
function npmBinScript(script) {
    return path.resolve(
        __dirname,
        `../node_modules/.bin/` +
            (process.platform === "win32" ? `${script}.cmd` : script)
    );
}
