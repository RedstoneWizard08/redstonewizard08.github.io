...
export class WebglRenderer extends Disposable implements IRenderer {
    ...
    private _rectangleRenderer: MutableDisposable<RectangleRenderer> =
        this.register(new MutableDisposable());
    private _glyphRenderer: MutableDisposable<GlyphRenderer> = this.register(
        new MutableDisposable()
    );
    ...
    private readonly _onChangeTextureAtlas = this.register(
    ...
    private readonly _onRemoveTextureAtlasCanvas = this.register(
        new EventEmitter<HTMLCanvasElement>()
    );
    public readonly onRemoveTextureAtlasCanvas =
        this._onRemoveTextureAtlasCanvas.event;
    private readonly _onRequestRedraw = this.register(
        new EventEmitter<IRequestRedrawEvent>()
    );
    ...
    constructor(
        ...
        this.register(
            this._themeService.onChangeColors(() => this._handleColorChange())
        );
        ...
        this.register(
            _optionsService.onOptionChange(() => this._handleOptionsChanged())
        );
        ...
        this._gl = this._canvas.getContext(
            "webgl2",
            contextAttributes
        ) as IWebGL2RenderingContext;
        if (!this._gl) {
            throw new Error("WebGL2 not supported " + this._gl);
        }

        this.register(
            addDisposableDomListener(this._canvas, "webglcontextlost", (e) => {
    ...
    private _refreshCharAtlas(): void {
        ...
        if (this._charAtlas !== atlas) {
            ...
            this._charAtlasDisposable.value = getDisposeArrayDisposable([
                forwardEvent(
                    atlas.onAddTextureAtlasCanvas,
                    this._onAddTextureAtlasCanvas
                ),
                forwardEvent(
                    atlas.onRemoveTextureAtlasCanvas,
    ...
    public registerCharacterJoiner(
    ...
    private _updateModel(start: number, end: number): void {
        ...
        for (y = start; y <= end; y++) {
            ...
            joinedRanges =
                this._characterJoinerService.getJoinedCharacters(row);
            for (x = 0; x < terminal.cols; x++) {
                ...
                if (
                    ...
                    this._model.cells[i + RENDER_MODEL_BG_OFFSET] ===
                        this._cellColorResolver.result.bg &&
                    this._model.cells[i + RENDER_MODEL_FG_OFFSET] ===
                        this._cellColorResolver.result.fg &&
                    this._model.cells[i + RENDER_MODEL_EXT_OFFSET] ===
                ...
                if (isJoined) {
                    ...
                    for (x++; x < lastCharX; x++) {
                        ...
                        this._model.cells[j + RENDER_MODEL_BG_OFFSET] =
                            this._cellColorResolver.result.bg;
                        this._model.cells[j + RENDER_MODEL_FG_OFFSET] =
                            this._cellColorResolver.result.fg;
                        this._model.cells[j + RENDER_MODEL_EXT_OFFSET] =
                            this._cellColorResolver.result.ext;
                    }
                }
            }
    ...
    private _setCanvasDevicePixelDimensions(
        width: number,
        height: number
    ): void {
