...
export class GlyphRenderer extends Disposable {
    ...
    constructor(
        ...
        if (TextureAtlas.maxAtlasPages === undefined) {
            // Typically 8 or 16
            TextureAtlas.maxAtlasPages = Math.min(
                32,
                throwIfFalsy(
            ...
            TextureAtlas.maxTextureSize = throwIfFalsy(
                gl.getParameter(gl.MAX_TEXTURE_SIZE) as number | null
            );
        }
        ...
        this._program = throwIfFalsy(
            createProgram(
                gl,
                vertexShaderSource,
                createFragmentShaderSource(TextureAtlas.maxAtlasPages)
            )
        );
        this.register(toDisposable(() => gl.deleteProgram(this._program)));

        // Uniform locations
        this._projectionLocation = throwIfFalsy(
            gl.getUniformLocation(this._program, "u_projection")
        );
        this._resolutionLocation = throwIfFalsy(
            gl.getUniformLocation(this._program, "u_resolution")
        );
        this._textureLocation = throwIfFalsy(
            gl.getUniformLocation(this._program, "u_texture")
        );
        ...
        this.register(
            toDisposable(() => gl.deleteBuffer(unitQuadVerticesBuffer))
        );
        gl.bindBuffer(gl.ARRAY_BUFFER, unitQuadVerticesBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, unitQuadVertices, gl.STATIC_DRAW);
        gl.enableVertexAttribArray(VertexAttribLocations.UNIT_QUAD);
        gl.vertexAttribPointer(
            VertexAttribLocations.UNIT_QUAD,
            2,
            this._gl.FLOAT,
            false,
            0,
        ...
        this.register(
            toDisposable(() => gl.deleteBuffer(elementIndicesBuffer))
        );
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, elementIndicesBuffer);
        gl.bufferData(
            gl.ELEMENT_ARRAY_BUFFER,
            unitQuadElementIndices,
            gl.STATIC_DRAW
        );
        ...
        this.register(
            toDisposable(() => gl.deleteBuffer(this._attributesBuffer))
        );
        gl.bindBuffer(gl.ARRAY_BUFFER, this._attributesBuffer);
        gl.enableVertexAttribArray(VertexAttribLocations.OFFSET);
        gl.vertexAttribPointer(
            VertexAttribLocations.OFFSET,
            2,
            gl.FLOAT,
            false,
            BYTES_PER_CELL,
        ...
        gl.vertexAttribPointer(
            VertexAttribLocations.TEXCOORD,
            2,
            gl.FLOAT,
            false,
            BYTES_PER_CELL,
        ...
        gl.vertexAttribPointer(
            VertexAttribLocations.CELL_POSITION,
            2,
            gl.FLOAT,
            false,
            BYTES_PER_CELL,
        ...
        for (let i = 0; i < TextureAtlas.maxAtlasPages; i++) {
            ...
            this.register(
                toDisposable(() => gl.deleteTexture(glTexture.texture))
            );
    ...
    private _updateCell(
        ...
        if (
            code === NULL_CELL_CODE ||
            code ===
                undefined /* This is used for the right side of wide chars */
        ) {
            array.fill(
                0,
                $i,
                $i + INDICES_PER_CELL - 1 - CELL_POSITION_INDICES
            );
            return;
        }
        ...
        if (chars && chars.length > 1) {
            $glyph = this._atlas.getRasterizedGlyphCombinedChar(
                chars,
                bg,
                fg,
                ext,
        ...
        if (bg !== lastBg && $glyph.offset.x > $leftCellPadding) {
            ...
            array[$i + 5] =
                ...
                $clippedPixels /
                    this._atlas.pages[$glyph.texturePage].canvas.width;
            ...
            // a_size
            array[$i + 2] =
                $glyph.size.x / this._dimensions.device.canvas.width;
            array[$i + 3] =
                $glyph.size.y / this._dimensions.device.canvas.height;
            // a_texpage
            array[$i + 4] = $glyph.texturePage;
        ...
        if (this._optionsService.rawOptions.rescaleOverlappingGlyphs) {
            if (
                ...
                array[$i + 2] =
                    ...
                    this._dimensions.device.canvas.width; // - 1 to improve readability
            }
        }
    }
    ...
    public clear(): void {
        ...
        for (; i < this._vertices.attributesBuffers.length; i++) {
            if (this._vertices.count !== newCount) {
                this._vertices.attributesBuffers[i] = new Float32Array(
                    newCount
                );
            } else {
    ...
    public render(renderModel: IRenderModel): void {
        ...
        for (let y = 0; y < renderModel.lineLengths.length; y++) {
            ...
            const sub = this._vertices.attributes.subarray(
                si,
                si + renderModel.lineLengths[y] * INDICES_PER_CELL
            );
        ...
        gl.bufferData(
            gl.ARRAY_BUFFER,
            activeBuffer.subarray(0, bufferLength),
            gl.STREAM_DRAW
        );
        ...
        for (let i = 0; i < this._atlas.pages.length; i++) {
            if (
                this._atlas.pages[i].version !== this._atlasTextures[i].version
            ) {
