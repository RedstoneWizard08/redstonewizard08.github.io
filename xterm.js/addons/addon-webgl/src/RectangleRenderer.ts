...
const BYTES_PER_RECTANGLE =
    INDICES_PER_RECTANGLE * Float32Array.BYTES_PER_ELEMENT;
...
export class RectangleRenderer extends Disposable {
    ...
    constructor(
        ...
        gl.vertexAttribPointer(
            VertexAttribLocations.POSITION,
            2,
            gl.FLOAT,
            false,
            BYTES_PER_RECTANGLE,
    ...
    public updateBackgrounds(model: IRenderModel): void {
        ...
        for (y = 0; y < terminal.rows; y++) {
            ...
            for (x = 0; x < terminal.cols; x++) {
                modelIndex =
                    (y * terminal.cols + x) * RENDER_MODEL_INDICIES_PER_CELL;
                ...
                if (
                    bg !== currentBg ||
                    (fg !== currentFg && (currentInverse || inverse))
                ) {
                    // A rectangle needs to be drawn if going from non-default to another color
                    if (
                        currentBg !== 0 ||
                        (currentInverse && currentFg !== 0)
                    ) {
    ...
    public updateCursor(model: IRenderModel): void {
        ...
        if (cursor.style === "underline" || cursor.style === "outline") {
            ...
            this._addRectangleFloat(
                ...
                offset,
                cursor.x * this._dimensions.device.cell.width,
                (cursor.y + 1) * this._dimensions.device.cell.height -
                    cursor.dpr,
        ...
        if (cursor.style === "outline") {
            ...
            this._addRectangleFloat(
                vertices.attributes,
                offset,
                (cursor.x + cursor.width) * this._dimensions.device.cell.width -
                    cursor.dpr,
                cursor.y * this._dimensions.device.cell.height,
                cursor.dpr,
                this._dimensions.device.cell.height,
    ...
    private _updateRectangle(
        ...
        this._addRectangle(
            ...
            $x1,
            $y1,
            (endX - startX) * this._dimensions.device.cell.width,
            this._dimensions.device.cell.height,
            $r,
            $g,
            $b,
