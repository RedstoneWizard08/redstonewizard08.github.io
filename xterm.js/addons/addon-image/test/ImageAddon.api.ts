...
const TESTDATA_IIP: [string, [number, number]][] = [
    [
        readFileSync("./addons/addon-image/fixture/iip/palette.iip", {
            encoding: "utf-8",
        }),
        [640, 80],
    ],
    [
        readFileSync("./addons/addon-image/fixture/iip/spinfox.iip", {
            encoding: "utf-8",
        }),
        [148, 148],
    ],
    [
        readFileSync("./addons/addon-image/fixture/iip/w3c_gif.iip", {
            encoding: "utf-8",
        }),
        [72, 48],
    ],
    [
        readFileSync("./addons/addon-image/fixture/iip/w3c_jpg.iip", {
            encoding: "utf-8",
        }),
        [72, 48],
    ],
    [
        readFileSync("./addons/addon-image/fixture/iip/w3c_png.iip", {
            encoding: "utf-8",
        }),
        [72, 48],
    ],
...
describe("ImageAddon", () => {
    ...
    it("test for private accessors", async () => {
        ...
        assert.equal(
            await page.evaluate(
                "window.term._core.buffer.lines.get(0)._data instanceof Uint32Array"
            ),
            true
        );
        assert.equal(
            await page.evaluate(
                "window.term._core.buffer.lines.get(0)._extendedAttrs instanceof Object"
            ),
            true
        );
        ...
        assert.equal(
            await page.evaluate(
                "window.term._core._inputHandler._curAttrData.constructor.name"
            ),
            "AttributeData"
        );
        assert.equal(
            await page.evaluate(
                "window.term._core._inputHandler._parser.constructor.name"
            ),
    ...
    describe("ctor options", () => {
        it("empty settings should load defaults", async () => {
            ...
            assert.deepEqual(
                await page.evaluate(`window.imageAddon._opts`),
                DEFAULT_OPTIONS
            );
        });
        it("custom settings should overload defaults", async () => {
            ...
            await page.evaluate(
                (opts) => {
                    (window as any).imageAddonCustom = new ImageAddon(
                        opts.opts
                    );
                    (window as any).term.loadAddon(
                        (window as any).imageAddonCustom
                    );
                },
                { opts: customSettings }
            );
            assert.deepEqual(
                await page.evaluate(`window.imageAddonCustom._opts`),
                customSettings
            );
        });
    });
    ...
    describe("scrolling & cursor modes", () => {
        it("testdata default (scrolling with VT240 cursor pos)", async () => {
            ...
            assert.deepEqual(await getCursor(), [
                0,
                Math.floor(TESTDATA.height / dim.cellHeight),
            ]);
            // moved to right by 10 cells
            await writeToTerminal("#".repeat(10) + SIXEL_SEQ_0);
            assert.deepEqual(await getCursor(), [
                10,
                Math.floor(TESTDATA.height / dim.cellHeight) * 2,
            ]);
        });
        ...
        it("testdata cursor always at VT240 pos", async () => {
            ...
            assert.deepEqual(await getCursor(), [
                10 + 30,
                Math.floor(TESTDATA.height / dim.cellHeight) * 3,
            ]);
        });
    });
    ...
    describe("image lifecycle & eviction", () => {
        it("delete image once scrolled off", async () => {
            ...
            await page.evaluate(
                () =>
                    new Promise((res) => (window as any).term.write("\n", res))
            );
        ...
        it("get storageUsage", async () => {
            ...
            await writeToTerminal(SIXEL_SEQ_0);
            assert.closeTo(
                await page.evaluate("imageAddon.storageUsage"),
                (640 * 80 * 4) / 1000000,
                0.05
            );
        });
        ...
        it("set storageLimit removes images synchronously", async () => {
            ...
            const usage: number = await page.evaluate(
                "imageAddon.storageUsage"
            );
            const newUsage: number = await page.evaluate(
                "imageAddon.storageLimit = 0.5; imageAddon.storageUsage"
            );
...
async function getCursor(): Promise<[number, number]> {
    return page.evaluate(
        "[window.term.buffer.active.cursorX, window.term.buffer.active.cursorY]"
    );
}
...
async function writeToTerminal(d: string): Promise<any> {
    return page.evaluate(
        (data) => new Promise((res) => (window as any).term.write(data, res)),
        d
    );
}
