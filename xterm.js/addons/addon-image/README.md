...
- _IMPORTANT:_ The worker approach as done in previous releases got removed.
...
- **SIXEL Palette Handling**  
  ...
  The limit can be increased to a maximum of 4096 registers (via `sixelPaletteLimit`).
    ...
    (_printer mode_), thus it is technically possible to use more colors in one image than the palette has color slots.
    This feature is called _high-color_ in libsixel.
