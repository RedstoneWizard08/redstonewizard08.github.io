...
export class ImageStorage implements IDisposable {
    ...
    public setLimit(value: number): void {
        if (value < 0.5 || value > 1000) {
            throw RangeError(
                "invalid storageLimit, should be at least 0.5 MB and not exceed 1G"
            );
        }
                ...
                if (line.getBg(col) & BgFlags.HAS_EXTENDED) {
                    ...
                    if (e.tileId !== -1) {
                        ...
                        if (imgSpec) {
                            ...
                            this._renderer.drawPlaceholder(
                                startCol,
                                row,
                                count
                            );
    ...
    public viewportResize(metrics: { cols: number; rows: number }): void {
        ...
        for (let row = 0; row < rows; ++row) {
            ...
            if (line.getBg(oldCol) & BgFlags.HAS_EXTENDED) {
                const e: IExtendedAttrsImage =
                    line._extendedAttrs[oldCol] || EMPTY_ATTRS;
                const imageId = e.imageId;
                if (imageId === undefined || imageId === -1) {
                    continue;
                }
                ...
                const tilesPerRow = Math.ceil(
                    (imgSpec.actual?.width || 0) / imgSpec.actualCellSize.width
                );
                ...
                let hasData = false;
                for (
                    let rightCol = oldCol + 1;
                    rightCol > metrics.cols;
                    ++rightCol
                ) {
                    if (
                        line._data[rightCol * Cell.SIZE + Cell.CONTENT] &
                        Content.HAS_CONTENT_MASK
                    ) {
                        hasData = true;
                        break;
                    }
                ...
                const end = Math.min(
                    metrics.cols,
                    tilesPerRow - (e.tileId % tilesPerRow) + oldCol
                );
    ...
    public getImageAtBufferCell(
        ...
        if (line && line.getBg(x) & BgFlags.HAS_EXTENDED) {
            const e: IExtendedAttrsImage =
                line._extendedAttrs[x] || EMPTY_ATTRS;
            if (e.imageId && e.imageId !== -1) {
                ...
                if (window.ImageBitmap && orig instanceof ImageBitmap) {
                    ...
                    canvas
                        .getContext("2d")
                        ?.drawImage(orig, 0, 0, orig.width, orig.height);
                    return canvas;
                }
    ...
    private _writeToCell(
        ...
        if (line._data[x * Cell.SIZE + Cell.BG] & BgFlags.HAS_EXTENDED) {
            ...
            if (old) {
                ...
                line._extendedAttrs[x] = new ExtendedAttrsImage(
                    old.ext,
                    old.urlId,
                    imageId,
                    tileId
