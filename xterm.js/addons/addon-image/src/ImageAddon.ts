...
export class ImageAddon implements ITerminalAddon, IImageApi {
    ...
    public activate(terminal: ITerminalExt): void {
        ...
        this._disposeLater(
            ...
            terminal.parser.registerCsiHandler(
                { prefix: "?", final: "h" },
                (params) => this._decset(params)
            ),
            terminal.parser.registerCsiHandler(
                { prefix: "?", final: "l" },
                (params) => this._decrst(params)
            ),
            ...
            terminal.parser.registerCsiHandler(
                { prefix: "?", final: "S" },
                (params) => this._xtermGraphicsAttributes(params)
            ),
            ...
            terminal.buffer.onBufferChange(() =>
                this._storage?.wipeAlternate()
            ),

            // extend images to the right on resize
            terminal.onResize((metrics) =>
                this._storage?.viewportResize(metrics)
            )
        );

        // SIXEL handler
        if (this._opts.sixelSupport) {
            const sixelHandler = new SixelHandler(
                this._opts,
                this._storage!,
                terminal
            );
        ...
        if (this._opts.iipSupport) {
            const iipHandler = new IIPHandler(
                this._opts,
                this._renderer!,
                this._storage!,
                terminal
            ...
            this._disposeLater(
                terminal._core._inputHandler._parser.registerOscHandler(
                    1337,
                    iipHandler
                )
            );
    ...
    public extractTileAtBufferCell(
        x: number,
        y: number
    ): HTMLCanvasElement | undefined {
    ...
    private _xtermGraphicsAttributes(params: (number | number[])[]): boolean {
        ...
        if (params[0] === GaItem.COLORS) {
            switch (params[1]) {
                case GaAction.READ:
                    this._report(
                        `\x1b[?${params[0]};${GaStatus.SUCCESS};${this._opts.sixelPaletteLimit}S`
                    );
                    return true;
                case GaAction.SET_DEFAULT:
                    this._opts.sixelPaletteLimit =
                        this._defaultOpts.sixelPaletteLimit;
                    this._report(
                        `\x1b[?${params[0]};${GaStatus.SUCCESS};${this._opts.sixelPaletteLimit}S`
                    );
                ...
                case GaAction.SET:
                    if (
                        params.length > 2 &&
                        !(params[2] instanceof Array) &&
                        params[2] <= MAX_SIXEL_PALETTE_SIZE
                    ) {
                        ...
                        this._report(
                            `\x1b[?${params[0]};${GaStatus.ACTION_ERROR}S`
                        );
                    }
                    return true;
                case GaAction.READ_MAX:
                    this._report(
                        `\x1b[?${params[0]};${GaStatus.SUCCESS};${MAX_SIXEL_PALETTE_SIZE}S`
                    );
                    return true;
                default:
                    this._report(
