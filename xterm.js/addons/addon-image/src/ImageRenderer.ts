...
export class ImageRenderer extends Disposable implements IDisposable {
    ...
    public static createImageData(
        ...
        if (typeof ImageData !== "function") {
            ...
            if (buffer) {
                imgData.data.set(
                    new Uint8ClampedArray(buffer, 0, width * height * 4)
                );
            }
            return imgData;
        }
    ...
    public static createImageBitmap(
    ...
    constructor(private _terminal: ITerminalExt) {
        ...
        this._optionsRefresh.value =
            this._terminal._core.optionsService.onOptionChange((option) => {
                if (option === "fontSize") {
                    this.rescaleCanvas();
        ...
        if (value) {
            if (!this._placeholder && this.cellSize.height !== -1) {
                this._createPlaceHolder(
                    Math.max(this.cellSize.height + 1, PLACEHOLDER_HEIGHT)
                );
            }
        } else {
            this._placeholderBitmap?.close();
    ...
    public draw(
        ...
        const finalWidth =
            count * width + sx > img.width ? img.width - sx : count * width;
    ...
    public drawPlaceholder(col: number, row: number, count: number = 1): void {
        if (this._ctx) {
            ...
            if (!this._placeholder) {
                this._createPlaceHolder(
                    Math.max(height + 1, PLACEHOLDER_HEIGHT)
                );
    ...
    public rescaleCanvas(): void {
        ...
        if (
            this.canvas.width !== this.dimensions!.css.canvas.width ||
            this.canvas.height !== this.dimensions!.css.canvas.height
        ) {
     ...
     */
    private _rescaleImage(
        spec: IImageSpec,
        currentWidth: number,
        currentHeight: number
    ): void {
        if (
            currentWidth === spec.actualCellSize.width &&
            currentHeight === spec.actualCellSize.height
        ) {
            return;
        }
    ...
    private _open(): void {
        ...
        this._oldSetRenderer = this._renderService.setRenderer.bind(
            this._renderService
        );
    ...
    public insertLayerToDom(): void {
        ...
        if (this.document && this._terminal._core.screenElement) {
            ...
            console.warn(
                "image addon: cannot insert output canvas to DOM, missing document or screenElement"
            );
        }
    }
    ...
    private _createPlaceHolder(height: number = PLACEHOLDER_HEIGHT): void {
        ...
        const blueprint = ImageRenderer.createCanvas(
            this.document,
            bWidth,
            height
        );
        ...
        const width =
            (screen.width + bWidth - 1) & ~(bWidth - 1) || PLACEHOLDER_LENGTH;
        this._placeholder = ImageRenderer.createCanvas(
            this.document,
            width,
            height
        );
        ...
        ImageRenderer.createImageBitmap(this._placeholder).then(
            (bitmap) => (this._placeholderBitmap = bitmap)
        );
    }
