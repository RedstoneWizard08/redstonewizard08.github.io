...
export class SixelHandler implements IDcsHandler, IResetHandler {
    ...
    public unhook(success: boolean): boolean | Promise<boolean> {
        ...
        canvas
            .getContext("2d")
            ?.putImageData(new ImageData(this._dec.data8, width, height), 0, 0);
...
function extractActiveBg(
    attr: AttributeData,
    colors: ReadonlyColorSet | undefined
): RGBA8888 {
    let bg = 0;
    if (!colors) {
    ...
    if (attr.isInverse()) {
        if (attr.isFgDefault()) {
            ...
            const t = (attr.constructor as typeof AttributeData).toColorRGB(
                attr.getFgColor()
            );
            bg = toRGBA8888(...t);
        } else {
