...
describe("parse", () => {
    ...
    it("parses multiple families", () => {
        assert.deepEqual(parse("Arial, Verdana, serif"), [
            "Arial",
            "Verdana",
            "serif",
        ]);
    ...
    it("parses quoted families", () => {
        assert.deepEqual(parse('"Times New Roman", serif'), [
            "Times New Roman",
            "serif",
        ]);
    });
    ...
    it("handles escaped characters in strings", () => {
        assert.deepEqual(
            parse('"quote \\" slash \\\\ slashquote \\\\\\"", serif'),
            ['quote " slash \\ slashquote \\"', "serif"]
        );
    });
    ...
    it("handles unicode escape sequences", () => {
        assert.deepEqual(parse('"space\\20 between", serif'), [
            "space between",
            "serif",
        ]);
    });
    ...
    it("swallows only the first space after a unicode escape", () => {
        assert.deepEqual(parse('"two-space\\20  between", serif'), [
            "two-space  between",
            "serif",
        ]);
    });
    ...
    it("automatically ends the unicode escape after six digits", () => {
        assert.deepEqual(parse("space\\000020between, serif"), [
            "space between",
            "serif",
        ]);
    });
    ...
    it("handles unicode escapes at the end of the family", () => {
        assert.deepEqual(parse("endswithbrace \\7b, serif"), [
            "endswithbrace {",
            "serif",
        ]);
    });
