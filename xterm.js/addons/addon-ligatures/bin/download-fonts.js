...
async function download() {
    ...
    try {
        ...
        console.warn(
            "Fonts failed to download, ligature tests will not work",
            e
        );
    }
}
...
async function downloadIosevka() {
    ...
    if (await util.promisify(fs.exists)(file)) {
        ...
        const iosevkaContents = (
            await axios.get(urls.iosevka, { responseType: "arraybuffer" })
        ).data;
        const iosevkaZipfile = await util.promisify(yauzl.fromBuffer)(
            iosevkaContents
        );
