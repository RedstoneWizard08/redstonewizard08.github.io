...
export class TextRenderLayer extends BaseRenderLayer {
    ...
    constructor(
        ...
        this.register(
            optionsService.onSpecificOptionChange(
                "allowTransparency",
                (value) => this._setTransparency(value)
            )
        );
    }
    ...
    private _forEachCell(
        firstRow: number,
        lastRow: number,
        callback: (cell: ICellData, x: number, y: number) => void
    ): void {
            ...
            for (let x = 0; x < this._bufferService.cols; x++) {
                ...
                if (!isJoined && this._isOverlapping(cell)) {
                    ...
                    if (
                        ...
                        line!.getCodePoint(lastCharX + 1) === NULL_CELL_CODE
                    ) {
                        // patch width to 2
        ...
        this._forEachCell(firstRow, lastRow, (cell, x, y) => {
            ...
            if (cell.isInverse()) {
                if (cell.isFgDefault()) {
                    ...
                    nextFillStyle =
                        this._themeService.colors.ansi[cell.getFgColor()].css;
                }
            } else if (cell.isBgRGB()) {
                nextFillStyle = `rgb(${AttributeData.toColorRGB(cell.getBgColor()).join(",")})`;
            } else if (cell.isBgPalette()) {
                nextFillStyle =
                    this._themeService.colors.ansi[cell.getBgColor()].css;
            }
