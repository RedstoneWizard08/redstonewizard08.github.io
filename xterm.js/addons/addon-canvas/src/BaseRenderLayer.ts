    ...
    implements IRenderLayer
{
    ...
    public get cacheCanvas(): HTMLCanvasElement {
        return this._charAtlas?.pages[0].canvas!;
    }

    private readonly _onAddTextureAtlasCanvas = this.register(
    ...
    private _refreshCharAtlas(colorSet: ReadonlyColorSet): void {
        ...
        this._charAtlas = acquireTextureAtlas(
            this._terminal,
        ...
        this._charAtlasDisposable.value = forwardEvent(
            this._charAtlas.onAddTextureAtlasCanvas,
            this._onAddTextureAtlasCanvas
        );
        this._charAtlas.warmUp();
        for (let i = 0; i < this._charAtlas.pages.length; i++) {
            this._bitmapGenerator[i] = new BitmapGenerator(
                this._charAtlas.pages[i].canvas
            );
        }
    }
     ...
     */
    protected _fillMiddleLineAtCells(
        x: number,
        y: number,
        width: number = 1
    ): void {
     ...
     * @param y The row to fill.
     */
    protected _fillBottomLineAtCells(
        x: number,
        y: number,
        width: number = 1,
        ...
        this._ctx.fillRect(
            ...
            (y + 1) * this._deviceCellHeight +
                pixelOffset -
                this._coreBrowserService.dpr -
                1 /* Ensure it's drawn within the cell */,
            width * this._deviceCellWidth,
            this._coreBrowserService.dpr
        );
    }

    protected _curlyUnderlineAtCell(
        x: number,
        y: number,
        width: number = 1
    ): void {
    ...
    protected _dottedUnderlineAtCell(
        x: number,
        y: number,
        width: number = 1
    ): void {
    ...
    protected _dashedUnderlineAtCell(
        x: number,
        y: number,
        width: number = 1
    ): void {
    ...
    protected _drawChars(cell: ICellData, x: number, y: number): void {
        ...
        this._cellColorResolver.resolve(
            cell,
            x,
            this._bufferService.buffer.ydisp + y,
            this._deviceCellWidth
        );
        ...
        if (chars && chars.length > 1) {
            glyph = this._charAtlas.getRasterizedGlyphCombinedChar(
                chars,
            ...
            glyph = this._charAtlas.getRasterizedGlyph(
                cell.getCode() || WHITESPACE_CELL_CODE,
                this._cellColorResolver.result.bg,
                this._cellColorResolver.result.fg,
                this._cellColorResolver.result.ext,
                true
        ...
        if (
            this._bitmapGenerator[glyph.texturePage] &&
            this._charAtlas.pages[glyph.texturePage].canvas !==
                this._bitmapGenerator[glyph.texturePage]!.canvas
        ) {
        ...
        if (
            this._charAtlas.pages[glyph.texturePage].version !==
            this._bitmapGenerator[glyph.texturePage]?.version
        ) {
            if (!this._bitmapGenerator[glyph.texturePage]) {
                this._bitmapGenerator[glyph.texturePage] = new BitmapGenerator(
                    this._charAtlas.pages[glyph.texturePage].canvas
                );
            }
        ...
        if (this._optionsService.rawOptions.rescaleOverlappingGlyphs) {
            if (
                allowRescaling(code, width, glyph.size.x, this._deviceCellWidth)
            ) {
        ...
        this._ctx.drawImage(
            this._bitmapGenerator[glyph.texturePage]?.bitmap ||
                this._charAtlas!.pages[glyph.texturePage].canvas,
...
class BitmapGenerator {
    ...
    public get bitmap(): ImageBitmap | undefined {
        return this._bitmap;
    }
