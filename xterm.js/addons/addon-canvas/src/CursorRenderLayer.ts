...
export class CursorRenderLayer extends BaseRenderLayer {
    ...
    private _cursorRenderers: {
        [key: string]: (x: number, y: number, cell: ICellData) => void;
    };
    ...
    private _render(triggeredByAnimationFrame: boolean): void {
        ...
        const viewportRelativeCursorY =
            cursorY - this._bufferService.buffer.ydisp;

        // Don't draw the cursor if it's off-screen
        if (
            viewportRelativeCursorY < 0 ||
            viewportRelativeCursorY >= this._bufferService.rows
        ) {
            this._clearCursor();
            return;
        }
        ...
        if (!this._coreBrowserService.isFocused) {
            ...
            if (cursorInactiveStyle && cursorInactiveStyle !== "none") {
                this._cursorRenderers[cursorInactiveStyle](
                    cursorX,
        ...
        ](cursorX, viewportRelativeCursorY, this._cell);
        this._ctx.restore();
