    ...
    describe("correct buffer offsets & uri", () => {
      ...
      `);
        ...
        it("all half width", async () => {
            ...
            await evalLinkStateData("http://example.com", {
                start: { x: 5, y: 1 },
                end: { x: 22, y: 1 },
            });
        ...
        it("full width within url and before", async () => {
            ...
            await resetAndHover(8, 0);
            await evalLinkStateData(
                "https://ko.wikipedia.org/wiki/위키백과:대문",
                { start: { x: 8, y: 1 }, end: { x: 11, y: 2 } }
            );
            await resetAndHover(1, 1);
            await evalLinkStateData(
                "https://ko.wikipedia.org/wiki/위키백과:대문",
...
async function pollForLinkAtCell(
    ...
    await pollFor(
        page,
        `!!Array.from(document.querySelectorAll('.xterm-rows > :nth-child(${row + 1}) > span[style]')).filter(el => el.style.textDecoration == 'underline').length`,
        true
    );
    const text = await page.evaluate(
        `Array.from(document.querySelectorAll('.xterm-rows > :nth-child(${row + 1}) > span[style]')).filter(el => el.style.textDecoration == 'underline').map(el => el.textContent).join('');`
    );
...
async function cellPos(col: number, row: number): Promise<[number, number]> {
  ...
  `);
    return [
        col * coords.width + coords.left + 2,
        row * coords.height + coords.top + 2,
    ];
}
