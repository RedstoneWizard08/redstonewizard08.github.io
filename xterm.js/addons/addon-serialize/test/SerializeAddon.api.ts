...
const writeRawSync = (page: any, str: string): Promise<void> =>
    writeSync(page, `' +` + JSON.stringify(str) + `+ '`);

const testNormalScreenEqual = async (page: any, str: string): Promise<void> => {
    await writeRawSync(page, str);
    const originalBuffer = await page.evaluate(
        `inspectBuffer(term.buffer.normal);`
    );
...
async function testSerializeEquals(
    writeContent: string,
...
describe("SerializeAddon", () => {
    ...
    it("produce different output when we call test util with different text", async function (): Promise<any> {
        ...
        const buffer1 = await page.evaluate(
            `inspectBuffer(term.buffer.normal);`
        );

        await page.evaluate(`term.reset();`);
        await writeRawSync(page, "67890");
        const buffer2 = await page.evaluate(
            `inspectBuffer(term.buffer.normal);`
        );
    ...
    it("produce different output when we call test util with different line wrap", async function (): Promise<any> {
        ...
        const buffer3 = await page.evaluate(
            `inspectBuffer(term.buffer.normal);`
        );

        await page.evaluate(`term.reset();`);
        await writeRawSync(page, "123456789012345");
        const buffer4 = await page.evaluate(
            `inspectBuffer(term.buffer.normal);`
        );
    ...
    it("unwrap wrapped line", async function (): Promise<any> {
        ...
        assert.equal(
            await page.evaluate(`serializeAddon.serialize();`),
            lines.join("\r\n")
        );
    });
    ...
    it("serialize with half of scrollback", async function (): Promise<any> {
        ...
        const lines = newArray<string>(
            (index: number) => digitsString(cols, index),
            rows
        );
        await writeSync(page, lines.join("\\r\\n"));
        assert.equal(
            await page.evaluate(
                `serializeAddon.serialize({ scrollback: ${halfScrollback} });`
            ),
    ...
    it("serialize 0 rows of scrollback", async function (): Promise<any> {
        ...
        await writeSync(page, lines.join("\\r\\n"));
        assert.equal(
            await page.evaluate(`serializeAddon.serialize({ scrollback: 0 });`),
    ...
    it("serialize exclude alt buffer", async () => {
        ...
        assert.equal(
            await page.evaluate(`serializeAddon.serialize();`),
            "normal\x1b[?1049h\x1b[Halt"
        );
        assert.equal(
            await page.evaluate(
                `serializeAddon.serialize({ excludeAltBuffer: true });`
            ),
            "normal"
        );
    });
        ...
        const lines = newArray<string>(
            (index: number) =>
                digitsString(
                    cols,
                    index,
                    `\x1b[${color16[index % color16.length]}m`
                ),
            rows
        );
        await writeSync(page, lines.join("\\r\\n"));
    ...
    it("serialize all rows of content with color256", async function (): Promise<any> {
        ...
        const lines = newArray<string>(
            (index: number) =>
                digitsString(cols, index, `\x1b[38;5;${16 + index}m`),
            rows
        );
        await writeSync(page, lines.join("\\r\\n"));
        assert.equal(
    ...
    it("serialize with alt screen correctly", async () => {
        ...
        await writeSync(page, lines.join("\\r\\n"));
        assert.equal(
            await page.evaluate(`window.term.buffer.active.type`),
            "alternate"
        );
        assert.equal(
            JSON.stringify(await page.evaluate(`serializeAddon.serialize();`)),
