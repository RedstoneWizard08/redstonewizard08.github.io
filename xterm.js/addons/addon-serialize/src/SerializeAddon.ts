    ...
    public serialize(
        ...
        for (let row = startRow; row <= endRow; row++) {
            ...
            if (line) {
                ...
                const endLineColumn =
                    row === range.end.y ? endColumn : line.length;
                for (let col = startLineColumn; col < endLineColumn; col++) {
                    const c = line.getCell(
                        col,
                        oldCell === cell1 ? cell2 : cell1
                    );
                    if (!c) {
                        console.warn(
                            `Can't get cell at row=${row}, col=${col}`
                        );
                        continue;
                    }
...
class StringSerializeHandler extends BaseSerializeHandler {
    ...
    protected _rowEnd(row: number, isLastRow: boolean): void {
        ...
        if (!isLastRow) {
            ...
            if (!nextLine.isWrapped) {
                ...
                const nextRowFirstChar = nextLine.getCell(
                    0,
                    this._nextRowFirstChar
                )!;
                const isNextRowFirstCharDoubleWidth =
                    nextRowFirstChar.getWidth() > 1;
                ...
                if (
                    ...
                    if (
                        ...
                        (thisRowLastChar.getChars() ||
                            thisRowLastChar.getWidth() === 0) &&
    ...
    private _diffStyle(
        cell: IBufferCell | IAttributeData,
        oldCell: IBufferCell
    ): number[] {
        const sgrSeq: number[] = [];
                ...
                if (bgChanged) {
                    ...
                    if (cell.isBgRGB()) {
                        ...
                        if (color >= 16) {
                            ...
                            sgrSeq.push(
                                color & 8 ? 100 + (color & 7) : 40 + (color & 7)
                            );
                        }
                    } else {
                        sgrSeq.push(49);
    ...
    protected _nextCell(
        ...
        if (isEmptyCell) {
            ...
            // update cursor
            this._lastContentCursorRow = this._lastCursorRow = row;
            this._lastContentCursorCol = this._lastCursorCol =
                col + cell.getWidth();
        }
    }
    ...
    protected _serializeString(excludeFinalCursorPosition: boolean): string {
        ...
        const curAttrData: IAttributeData = (this._terminal as any)._core
            ._inputHandler._curAttrData;
    ...
    public serialize(options?: ISerializeOptions): string {
        ...
        if (!options?.excludeAltBuffer) {
            if (this._terminal.buffer.active.type === "alternate") {
                const alternativeScreenContent =
                    this._serializeBufferByScrollback(
                        this._terminal,
...
export class HTMLSerializeHandler extends BaseSerializeHandler {
    ...
    constructor(
        ...
        if ((_terminal as any)._core._themeService) {
            this._ansiColors = (
                _terminal as any
            )._core._themeService.colors.ansi;
        } else {
    ...
    private _getHexColor(cell: IBufferCell, isFg: boolean): string | undefined {
        ...
        if (isFg ? cell.isFgRGB() : cell.isBgRGB()) {
            ...
            return (
                "#" +
                rgb.map((x) => this._padStart(x.toString(16), 2, "0")).join("")
            );
        }
