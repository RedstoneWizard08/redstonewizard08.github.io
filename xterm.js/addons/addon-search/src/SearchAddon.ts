...
export class SearchAddon
    extends Disposable
    implements ITerminalAddon, ISearchApi
{
    private _terminal: Terminal | undefined;
    private _cachedSearchTerm: string | undefined;
    private _highlightedLines: Set<number> = new Set();
    private _highlightDecorations: IHighlight[] = [];
    private _selectedDecoration: MutableDisposable<IHighlight> = this.register(
    ...
    private readonly _onDidChangeResults = this.register(
        new EventEmitter<{ resultIndex: number; resultCount: number }>()
    );
    ...
    constructor(options?: Partial<ISearchAddonOptions>) {
        ...
        this._highlightLimit =
            options?.highlightLimit ?? DEFAULT_HIGHLIGHT_LIMIT;
    }
    ...
    public findNext(term: string, searchOptions?: ISearchOptions): boolean {
        ...
        const didOptionsChanged = this._lastSearchOptions
            ? this._didOptionsChange(this._lastSearchOptions, searchOptions)
            : true;
    ...
    private _highlightAllMatches(
        ...
        for (const match of searchResultsWithHighlight) {
            const decoration = this._createResultDecoration(
                match,
    ...
    private _find(
        ...
        if (startCol > this._terminal.cols) {
            throw new Error(
                `Invalid col: ${startCol} to search in terminal of ${this._terminal.cols} cols`
            );
        }
        ...
        if (searchOptions?.decorations) {
            ...
            if (this._selectedDecoration.value) {
                ...
                for (let i = 0; i < this._highlightDecorations.length; i++) {
                    ...
                    if (
                        match.row === selectedMatch.row &&
                        match.col === selectedMatch.col &&
                        match.size === selectedMatch.size
                    ) {
                        resultIndex = i;
                        break;
                    }
    ...
    protected _findInLine(
            ...
            if (isReverseSearch) {
                ...
                while (
                    (foundTerm = searchRegex.exec(
                        searchStringLine.slice(0, offset)
                    ))
                ) {
                ...
                if (foundTerm && foundTerm[0].length > 0) {
                    resultIndex =
                        offset + (searchRegex.lastIndex - foundTerm[0].length);
                    term = foundTerm[0];
                }
            }
        } else {
        ...
        if (resultIndex >= 0) {
            ...
            while (
                startRowOffset < offsets.length - 1 &&
            ...
            const endColOffset =
                resultIndex + term.length - offsets[endRowOffset];
            const startColIndex = this._stringLengthToBufferSize(
                row + startRowOffset,
                startColOffset
            );
            const endColIndex = this._stringLengthToBufferSize(
                row + endRowOffset,
                endColOffset
            );
            const size =
    ...
    private _translateBufferLineToStringWithWrap(
        ...
        while (line) {
            ...
            if (lineWrapsToNext) {
                lineOffsets.push(
                    lineOffsets[lineOffsets.length - 1] + string.length
                );
            } else {
                break;
            }
        ...
        if (options) {
            ...
            if (marker) {
                ...
                if (decoration) {
                    ...
                    disposables.push(
                        decoration.onDispose(() => disposeArray(disposables))
                    );
        ...
        if (findResultDecoration) {
            ...
            disposables.push(
                findResultDecoration.onRender((e) =>
                    this._applyStyles(e, options.matchBorder, false)
                )
            );
            disposables.push(
                findResultDecoration.onDispose(() => disposeArray(disposables))
            );
        }
