...
export function infoToWidth(info: number, ambiguousIsWide = false): 0 | 1 | 2 {
    ...
    return v < CHARWIDTH_EA_AMBIGUOUS
        ? 1
        : v >= CHARWIDTH_WIDE || ambiguousIsWide
          ? 2
          : 1;
}
...
export function shouldJoin(beforeState: number, afterInfo: number): number {
    let beforeCode =
        (beforeState & GRAPHEME_BREAK_MASK) >> GRAPHEME_BREAK_SHIFT;
...
export function shouldJoinBackwards(
    beforeInfo: number,
    afterState: number
): number {
...
function _shouldJoin(beforeCode: number, afterCode: number): boolean {
    if (
        beforeCode >= GRAPHEME_BREAK_Hangul_L &&
