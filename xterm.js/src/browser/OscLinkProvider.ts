    ...
    public provideLinks(
        ...
        for (let x = 0; x < lineLength; x++) {
            ...
            if (finishLink || (currentStart !== -1 && x === lineLength - 1)) {
                const text =
                    this._oscLinkService.getLinkData(currentLinkId)?.uri;
                if (text) {
                    ...
                    if (!ignoreLink) {
                        ...
                        result.push({
                            ...
                            activate: (e, text) =>
                                linkHandler
                                    ? linkHandler.activate(e, text, range)
                                    : defaultActivate(e, text),
                            hover: (e, text) =>
                                linkHandler?.hover?.(e, text, range),
                            leave: (e, text) =>
                                linkHandler?.leave?.(e, text, range),
                        });
                    }
                }
                finishLink = false;
