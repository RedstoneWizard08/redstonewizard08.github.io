...
export class Linkifier extends Disposable implements ILinkifier2 {
    ...
    private _activeProviderReplies:
        | Map<Number, ILinkWithState[] | undefined>
        | undefined;
    private _activeLine: number = -1;

    private readonly _onShowLinkUnderline = this.register(
        new EventEmitter<ILinkifierEvent>()
    );
    public readonly onShowLinkUnderline = this._onShowLinkUnderline.event;
    private readonly _onHideLinkUnderline = this.register(
        new EventEmitter<ILinkifierEvent>()
    );
    ...
    constructor(
        ...
        this.register(
            addDisposableDomListener(this._element, "mouseleave", () => {
    ...
    private _askForLink(
        ...
        for (const [
            ...
            if (useLineCache) {
                ...
                if (existingReply) {
                    linkProvided = this._checkLinkProviderResult(
                        i,
                        position,
                        linkProvided
                    );
                ...
                linkProvider.provideLinks(
                    ...
                    (links: ILink[] | undefined) => {
                        ...
                        const linksWithState: ILinkWithState[] | undefined =
                            links?.map((link) => ({ link }));
    ...
    private _removeIntersectingLinks(
        ...
        for (let i = 0; i < replies.size; i++) {
            ...
            for (let i = 0; i < providerReply.length; i++) {
                ...
                const startX =
                    linkWithState.link.range.start.y < y
                        ? 0
                        : linkWithState.link.range.start.x;
                const endX =
    ...
    private _checkLinkProviderResult(
        ...
        for (let j = 0; j < index; j++) {
            if (
                !this._activeProviderReplies.has(j) ||
                this._activeProviderReplies.get(j)
            ) {
                hasLinkBefore = true;
            }
        }
        ...
        if (!hasLinkBefore && links) {
            const linkAtPosition = links.find((link) =>
                this._linkAtPosition(link.link, position)
            );
            if (linkAtPosition) {
                linkProvided = true;
    ...
    private _linkAtPosition(
        ...
        const lower =
            link.range.start.y * this._bufferService.cols + link.range.start.x;
        const upper =
            link.range.end.y * this._bufferService.cols + link.range.end.x;
    ...
    private _positionFromMouseEvent(
        ...
        return {
            x: coords[0],
            y: coords[1] + this._bufferService.buffer.ydisp,
        };
    }
