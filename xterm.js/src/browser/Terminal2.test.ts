...
const TESTFILES = glob.sync("**/escape_sequence_files/*.in", {
...
const FILES = TESTFILES.filter(
    (value) => !SKIP_FILES.includes(value.split("/").slice(-1)[0])
);
        ...
        (process.platform === "win32" ? it.skip : it)(
            ...
            async () => {
                ...
                const expected = fs.readFileSync(
                    filename.split(".")[0] + ".text",
                    "utf8"
                );
                const expectedRightTrimmed = expected
                    .split("\n")
                    .map((l) => l.replace(/\s+$/, ""))
                    .join("\n");
...
function terminalToString(term: Terminal): string {
    ...
    let lineText = "";
    for (
        let line = term.buffer.ybase;
        line < term.buffer.ybase + term.rows;
        line++
    ) {
