...
export class RenderService extends Disposable implements IRenderService {
    ...
    private _renderer: MutableDisposable<IRenderer> = this.register(
    ...
    private readonly _onDimensionsChange = this.register(
        new EventEmitter<IRenderDimensions>()
    );
    public readonly onDimensionsChange = this._onDimensionsChange.event;
    private readonly _onRenderedViewportChange = this.register(
        new EventEmitter<{ start: number; end: number }>()
    );
    public readonly onRenderedViewportChange =
        this._onRenderedViewportChange.event;
    private readonly _onRender = this.register(
        new EventEmitter<{ start: number; end: number }>()
    );
    public readonly onRender = this._onRender.event;
    private readonly _onRefreshRequest = this.register(
        new EventEmitter<{ start: number; end: number }>()
    );
    ...
    constructor(
        ...
        this.register(
            coreBrowserService.onDprChange(() =>
        ...
        this.register(
            optionsService.onOptionChange(() => this._handleOptionsChanged())
        );
        this.register(
        ...
        this.register(
            decorationService.onDecorationRegistered(() => this._fullRefresh())
        );
        this.register(
            decorationService.onDecorationRemoved(() => this._fullRefresh())
        );
        ...
        this._registerIntersectionObserver(
            coreBrowserService.window,
            screenElement
        );
        this.register(
            coreBrowserService.onWindowChange((w) =>
    ...
    public setRenderer(renderer: IRenderer): void {
        ...
        if (this._renderer.value) {
            this._renderer.value.onRequestRedraw((e) =>
                this.refreshRows(e.start, e.end, true)
            );
