...
describe("CharacterJoinerService", () => {
    ...
    beforeEach(() => {
        ...
        lines.set(
            5,
            lineData([["a", 0x11111111], [" -> b -> c -> "], ["d", 0x22222222]])
        );
        ...
        line6.resize(
            oldSize + sub.length,
            CellData.fromCharData([0, "", 0, 0])
        );
        for (let i = 0; i < sub.length; ++i)
            line6.setCell(i + oldSize, sub.loadCell(i, new CellData()));
        line6.resize(
            line6.length + 1,
            CellData.fromCharData([0, "\xf0\x9f\x98\x81", 1, 128513])
        );
        line6.resize(
            line6.length + 1,
    ...
    it("processes the input using all provided joiners", () => {
        service.register(substringJoiner("->"));
        assert.deepEqual(service.getJoinedCharacters(1), [
            [2, 4],
            [12, 14],
        ]);
    ...
    it("removes deregistered joiners from future calls", () => {
        ...
        assert.deepEqual(service.getJoinedCharacters(1), [[7, 9]]);
    ...
    it("doesn't process joins on differently-styled characters", () => {
        service.register(substringJoiner("->"));
        assert.deepEqual(service.getJoinedCharacters(2), [
            [2, 4],
            [12, 14],
        ]);
    });
    ...
    it("returns an empty list of ranges if there is nothing to be joined", () => {
        service.register(substringJoiner("->"));
        assert.deepEqual(service.getJoinedCharacters(3), []);
    });

    it("returns an empty list of ranges if the line is empty", () => {
        service.register(substringJoiner("->"));
        assert.deepEqual(service.getJoinedCharacters(4), []);
    });
    ...
    it("doesn't process same-styled ranges that only have one character", () => {
        ...
        assert.deepEqual(service.getJoinedCharacters(5), [[5, 6]]);
    });

    it("handles ranges that extend all the way to the end of the line", () => {
        service.register(substringJoiner("-> d"));
        assert.deepEqual(service.getJoinedCharacters(2), [[12, 16]]);
    });
    ...
    describe("range merging", () => {
        ...
        it("connects two ranges", () => {
            ...
            assert.deepEqual(service.getJoinedCharacters(0), [[0, 6]]);
        });
...
function lineData(data: IPartialLineData[]): IBufferLine {
    ...
    for (let i = 0; i < data.length; ++i) {
        ...
        tline.resize(
            tline.length + line.split("").length,
            CellData.fromCharData([0, "", 0, 0])
        );
        line.split("").map((char, idx) =>
            tline.setCell(
                idx + offset,
                CellData.fromCharData([attr, char, 1, char.charCodeAt(0)])
            )
        );
    }
    return tline;
