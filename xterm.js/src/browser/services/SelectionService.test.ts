...
describe("SelectionService", () => {
    ...
    function stringArrayToRow(chars: string[]): IBufferLine {
        ...
        chars.map((c, idx) =>
            line.setCell(idx, CellData.fromCharData([0, c, 1, c.charCodeAt(0)]))
        );
        return line;
    }
    ...
    describe("_selectLineAt", () => {
        it("should select the entire line", () => {
            ...
            assert.deepEqual(
                selectionService.model.finalSelectionStart,
                [0, 0]
            );
            assert.deepEqual(
                selectionService.model.finalSelectionEnd,
                [bufferService.cols, 0],
                "The actual selection spans the entire column"
            );
        });
    ...
    describe("selectLines", () => {
        it("should select a single line", () => {
            ...
            assert.deepEqual(selectionService.model.finalSelectionEnd, [
                bufferService.cols,
                1,
            ]);
        });
    ...
    describe("_areCoordsInSelection", () => {
        it("should return whether coords are in the selection", () => {
            assert.isFalse(
                selectionService.areCoordsInSelection([0, 0], [2, 0], [2, 1])
            );
            assert.isFalse(
                selectionService.areCoordsInSelection([1, 0], [2, 0], [2, 1])
            );
            assert.isTrue(
                selectionService.areCoordsInSelection([2, 0], [2, 0], [2, 1])
            );
            assert.isTrue(
                selectionService.areCoordsInSelection([10, 0], [2, 0], [2, 1])
            );
            assert.isTrue(
                selectionService.areCoordsInSelection([0, 1], [2, 0], [2, 1])
            );
            assert.isTrue(
                selectionService.areCoordsInSelection([1, 1], [2, 0], [2, 1])
            );
            assert.isFalse(
                selectionService.areCoordsInSelection([2, 1], [2, 0], [2, 1])
            );
        });
    });
});
