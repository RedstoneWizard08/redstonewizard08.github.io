...
export class CharSizeService extends Disposable implements ICharSizeService {
    ...
    private readonly _onCharSizeChange = this.register(
    ...
    constructor(
        ...
        try {
            this._measureStrategy = this.register(
                new TextMetricsMeasureStrategy(this._optionsService)
            );
        } catch {
            this._measureStrategy = this.register(
                new DomMeasureStrategy(
                    document,
                    parentElement,
...
abstract class BaseMeasureStategy
    extends Disposable
    implements IMeasureStrategy
{
    protected _result: IMeasureResult = { width: 0, height: 0 };

    protected _validateAndSet(
...
class DomMeasureStrategy extends BaseMeasureStategy {
    ...
    constructor(
        ...
        this._measureElement.textContent = "W".repeat(
            DomMeasureStrategyConstants.REPEAT
        );
    ...
    public measure(): Readonly<IMeasureResult> {
        ...
        this._validateAndSet(
            Number(this._measureElement.offsetWidth) /
                DomMeasureStrategyConstants.REPEAT,
