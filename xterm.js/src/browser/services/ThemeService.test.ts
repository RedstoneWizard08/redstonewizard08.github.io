...
import {
    ThemeService,
    DEFAULT_ANSI_COLORS,
} from "browser/services/ThemeService";
        ...
        it("should fill all colors with values", () => {
            for (const key of Object.keys(themeService.colors)) {
                if (
                    ...
                    // A #rrggbb or rgba(...)
                    assert.ok(
                        (themeService.colors as any)[key].css.length >= 7
                    );
                }
            }
    ...
    describe("setTheme", () => {
        ...
        it("should set all extended ansi colors in reverse order", () => {
            ...
            for (let ansiColor = 16; ansiColor <= 255; ansiColor++) {
                assert.equal(
                    themeService.colors.ansi[ansiColor].css,
        ...
        it("should set one extended ansi color and keep the other default", () => {
            ...
            assert.equal(
                themeService.colors.ansi[17].css,
                DEFAULT_ANSI_COLORS[17].css
            );
        });
        ...
        it("should set extended ansi colors to the default when they are unset", () => {
            ...
            assert.equal(
                themeService.colors.ansi[16].css,
                DEFAULT_ANSI_COLORS[16].css
            );
