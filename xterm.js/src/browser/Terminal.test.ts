...
import {
    MockViewport,
    MockCompositionHelper,
    MockRenderer,
    TestTerminal,
} from "browser/TestUtils.test";
...
describe("Terminal", () => {
    ...
    beforeEach(() => {
        ...
        term.viewport.onRequestScrollLines((e) =>
            term.scrollLines(
                e.amount,
    ...
    describe("clear", () => {
        it("should clear a buffer equal to rows", () => {
            const promptLine = term.buffer.lines.get(
                term.buffer.ybase + term.buffer.y
            );
            term.clear();
            ...
            for (let i = 1; i < term.rows; i++) {
                assert.deepEqual(
                    term.buffer.lines.get(i),
                    term.buffer.getBlankLine(DEFAULT_ATTR_DATA)
                );
            }
        });
        ...
        for (let i = 0xdc00; i <= 0xdcf0; i += 0x10) {
            ...
            it(`${range}: 2 characters per cell`, async function (): Promise<void> {
                ...
                for (let j = i; j <= i + 0xf; j++) {
                    ...
                    assert.equal(
                        tchar.getChars(),
                        high + String.fromCharCode(j)
                    );
                    assert.equal(tchar.getChars().length, 2);
                    assert.equal(tchar.getWidth(), 1);
                    assert.equal(
                        term.buffer.lines.get(0)!.loadCell(1, cell).getChars(),
                        ""
                    );
                    term.reset();
                }
            ...
            it(`${range}: 2 characters at last cell`, async () => {
                ...
                for (let j = i; j <= i + 0xf; j++) {
                    ...
                    assert.equal(
                        term.buffer.lines.get(1)!.loadCell(0, cell).getChars(),
                        ""
                    );
                    term.reset();
                }
            ...
            it(`${range}: 2 characters per cell over line end with autowrap`, async function (): Promise<void> {
                ...
                for (let j = i; j <= i + 0xf; j++) {
                    ...
                    assert.equal(
                        term.buffer.lines.get(1)!.loadCell(1, cell).getChars(),
                        ""
                    );
                    term.reset();
                }
            ...
            it(`${range}: 2 characters per cell over line end without autowrap`, async function (): Promise<void> {
                ...
                for (let j = i; j <= i + 0xf; j++) {
                    ...
                    const width = wcwidth(
                        (0xd800 - 0xd800) * 0x400 + j - 0xdc00 + 0x10000
                    );
                    if (width !== 1) {
                        continue;
                    }
    ...
    describe("insert mode", () => {
        ...
        it("halfwidth - all", async () => {
            ...
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(10, cell).getChars(),
                "a"
            );
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(14, cell).getChars(),
                "e"
            );
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(15, cell).getChars(),
                "0"
            );
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(79, cell).getChars(),
                "4"
            );
        });
        it("fullwidth - insert", async () => {
            ...
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(11, cell).getChars(),
                ""
            );
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(14, cell).getChars(),
        ...
        it("fullwidth - right border", async () => {
            ...
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(12, cell).getChars(),
                "￥"
            );
            assert.equal(
                term.buffer.lines.get(0)!.loadCell(79, cell).getChars(),
    ...
    describe("Windows Pty", () => {
        it("should mark lines as wrapped when the line ends in a non-null character after a LF", async () => {
            ...
            assert.equal(
                windowsModeTerminal.buffer.lines.get(0)!.isWrapped,
                false
            );
            assert.equal(
                windowsModeTerminal.buffer.lines.get(1)!.isWrapped,
                true,
                "This line should wrap in Windows mode as the previous line ends in a non-null character"
            );
            assert.equal(
                windowsModeTerminal.buffer.lines.get(2)!.isWrapped,
                false
            );
        });
    ...
    it("convertEol setting", async () => {
        ...
        assert.equal(
            termNotConverting.buffer.lines.get(0)!.translateToString(false),
            "Hello          "
        );
        assert.equal(
            termNotConverting.buffer.lines.get(1)!.translateToString(false),
            "     World     "
        );
        assert.equal(
            termNotConverting.buffer.lines.get(0)!.translateToString(true),
            "Hello"
        );
        assert.equal(
            termNotConverting.buffer.lines.get(1)!.translateToString(true),
            "     World"
        );

        // converting
        const termConverting = new TestTerminal({
            cols: 15,
            rows: 10,
            convertEol: true,
        });
        ...
        assert.equal(
            termConverting.buffer.lines.get(0)!.translateToString(false),
            "Hello          "
        );
        assert.equal(
            termConverting.buffer.lines.get(1)!.translateToString(false),
            "World          "
        );
        assert.equal(
            termConverting.buffer.lines.get(0)!.translateToString(true),
            "Hello"
        );
        assert.equal(
            termConverting.buffer.lines.get(1)!.translateToString(true),
            "World"
        );
    });
    ...
    describe("marker lifecycle", () => {
        ...
        it("should dispose on normal trim off the top", async () => {
            ...
            assert.deepEqual(
                disposeStack.map((el) => el.isDisposed),
                [true, true]
            );
            // trimmed markers should contain line -1
            assert.deepEqual(
                disposeStack.map((el) => el.line),
                [-1, -1]
            );
        });
