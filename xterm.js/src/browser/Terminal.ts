...
export class Terminal extends CoreTerminal implements ITerminal {
    ...
    private _accessibilityManager: MutableDisposable<AccessibilityManager> =
        this.register(new MutableDisposable());

    private readonly _onCursorMove = this.register(new EventEmitter<void>());
    public readonly onCursorMove = this._onCursorMove.event;
    private readonly _onKey = this.register(
        new EventEmitter<{ key: string; domEvent: KeyboardEvent }>()
    );
    public readonly onKey = this._onKey.event;
    private readonly _onRender = this.register(
        new EventEmitter<{ start: number; end: number }>()
    );
    public readonly onRender = this._onRender.event;
    private readonly _onSelectionChange = this.register(
    ...
    constructor(options: Partial<ITerminalOptions> = {}) {
        ...
        this._decorationService =
            this._instantiationService.createInstance(DecorationService);
        this._instantiationService.setService(
            IDecorationService,
            this._decorationService
        );
        this._linkProviderService =
            this._instantiationService.createInstance(LinkProviderService);
        this._instantiationService.setService(
            ILinkProviderService,
            this._linkProviderService
        );
        ...
        this.register(
            this._inputHandler.onRequestRefreshRows((start, end) =>
        ...
        this.register(
            this._inputHandler.onRequestWindowsOptionsReport((type) =>
                this._reportWindowsOptions(type)
            )
        );
        this.register(
            this._inputHandler.onColor((event) => this._handleColorEvent(event))
        );
        this.register(
                ...
                case ColorRequestType.SET:
                    if (acc === "ansi") {
                        this._themeService.modifyColors(
                            (colors) =>
                                (colors.ansi[req.index] = channels.toColor(
                                    ...req.color
                                ))
                        );
                    } else {
                        ...
                        this._themeService.modifyColors(
                            (colors) =>
                                (colors[narrowedAcc] = channels.toColor(
                                    ...req.color
                                ))
                        );
                    }
    ...
    private _syncTextArea(): void {
        ...
        const cursorTop =
            this.buffer.y * this._renderService.dimensions.css.cell.height;
        const cursorLeft =
            cursorX * this._renderService.dimensions.css.cell.width;
    ...
    private _bindKeys(): void {
        this.register(
            addDisposableDomListener(
                this.textarea!,
                "keyup",
                (ev: KeyboardEvent) => this._keyUp(ev),
                true
            )
        );
        this.register(
        ...
        this.register(
            addDisposableDomListener(
                this.textarea!,
                "input",
                (ev: InputEvent) => this._inputEvent(ev),
                true
            )
        );
        this.register(
    ...
    public open(parent: HTMLElement): void {
        ...
        this.register(
            addDisposableDomListener(
                this.screenElement,
                "mousemove",
                (ev: MouseEvent) => this.updateCursorStyle(ev)
            )
        );
        ...
        this._coreBrowserService = this.register(
            this._instantiationService.createInstance(
                ...
                (this._document ?? typeof window !== "undefined")
                    ? window.document
                    : (null as any)
            )
        );
        ...
        this.register(
            addDisposableDomListener(this.textarea, "focus", (ev: FocusEvent) =>
                this._handleTextAreaFocus(ev)
            )
        );
        this.register(
            addDisposableDomListener(this.textarea, "blur", () =>
        ...
        this._themeService =
            this._instantiationService.createInstance(ThemeService);
        this._instantiationService.setService(
            IThemeService,
            this._themeService
        );
        ...
        this.register(
            addDisposableDomListener(
                this.element,
                "mousedown",
                (e: MouseEvent) => this._selectionService!.handleMouseDown(e)
            )
        );
    ...
    public bindMouse(): void {
        ...
        function sendEvent(ev: MouseEvent | WheelEvent): boolean {
            ...
            const pos = self._mouseService!.getMouseReportCoords(
                ev,
                self.screenElement!
            );
            if (!pos) {
            ...
            switch ((ev as any).overrideType || ev.type) {
                ...
                case "wheel":
                    if (
                        ...
                        self._customWheelEventHandler(ev as WheelEvent) ===
                            false
                    ) {
                        return false;
                    }
                    const amount = self.viewport!.getLinesScrolled(
                        ev as WheelEvent
                    );
                    ...
                    action =
                        (ev as WheelEvent).deltaY < 0
                            ? CoreMouseAction.UP
                            : CoreMouseAction.DOWN;
            ...
            mouseup: (ev: MouseEvent) => {
                ...
                if (!ev.buttons) {
                    ...
                    this._document!.removeEventListener(
                        "mouseup",
                        requestedEvents.mouseup!
                    );
                    if (requestedEvents.mousedrag) {
                        this._document!.removeEventListener(
                            "mousemove",
                            requestedEvents.mousedrag
                        );
                    }
                }
            ...
            addDisposableDomListener(
                ...
                (ev: WheelEvent) => {
                    ...
                    if (
                        ...
                        this._customWheelEventHandler(ev) === false
                    ) {
                        return false;
                    }
    ...
    protected _keyDown(event: KeyboardEvent): boolean | undefined {
        ...
        if (
            ...
            !this._compositionHelper!.keydown(event)
        ) {
            if (
    ...
    private _reportWindowsOptions(type: WindowsOptionsReportType): void {
        ...
        switch (type) {
            case WindowsOptionsReportType.GET_WIN_SIZE_PIXELS:
                ...
                const canvasHeight =
                    this._renderService.dimensions.css.canvas.height.toFixed(0);
                this.coreService.triggerDataEvent(
                    `${C0.ESC}[4;${canvasHeight};${canvasWidth}t`
                );
                break;
            case WindowsOptionsReportType.GET_CELL_SIZE_PIXELS:
                const cellWidth =
                    this._renderService.dimensions.css.cell.width.toFixed(0);
                const cellHeight =
                    this._renderService.dimensions.css.cell.height.toFixed(0);
                this.coreService.triggerDataEvent(
                    `${C0.ESC}[6;${cellHeight};${cellWidth}t`
                );
                break;
        }
    }
