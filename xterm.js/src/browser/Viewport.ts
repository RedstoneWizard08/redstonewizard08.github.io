...
export class Viewport extends Disposable implements IViewport {
    ...
    private readonly _onRequestScrollLines = this.register(
        new EventEmitter<{ amount: number; suppressScrollEvent: boolean }>()
    );
    ...
    constructor(
        ...
        this.scrollBarWidth =
            this._viewportElement.offsetWidth - this._scrollArea.offsetWidth ||
        ...
        this.register(
            themeService.onChangeColors((e) => this._handleThemeChange(e))
        );
        this.register(
    ...
    private _bubbleScroll(ev: Event, amount: number): boolean {
        const scrollPosFromTop =
            this._viewportElement.scrollTop + this._lastRecordedViewportHeight;
        if (
            (amount < 0 && this._viewportElement.scrollTop !== 0) ||
        ...
        if (!this._optionsService.rawOptions.smoothScrollDuration) {
            ...
            if (this._smoothScrollPercent() < 1) {
                this._smoothScrollState.origin =
                    this._viewportElement.scrollTop;
                this._smoothScrollState.target =
                    this._smoothScrollState.origin + amount;
                this._smoothScrollState.target = Math.max(
                    Math.min(
                        this._smoothScrollState.target,
                        this._viewportElement.scrollHeight
