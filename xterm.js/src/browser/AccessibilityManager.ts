...
export class AccessibilityManager extends Disposable {
    ...
    constructor(
        ...
        this._topBoundaryFocusListener = (e) =>
            this._handleBoundaryFocus(e, BoundaryPosition.TOP);
        ...
        if (DEBUG) {
            ...
            this._debugRootContainer.appendChild(
                document.createTextNode("------start a11y------")
            );
            this._debugRootContainer.appendChild(this._accessibilityContainer);
            this._debugRootContainer.appendChild(
                document.createTextNode("------end a11y------")
            );
        ...
        this.register(
            this._terminal.onA11yChar((char) => this._handleChar(char))
        );
        this.register(this._terminal.onLineFeed(() => this._handleChar("\n")));
        this.register(
            this._terminal.onA11yTab((spaceCount) =>
        ...
        this.register(
            addDisposableDomListener(document, "selectionchange", () =>
    ...
    private _handleBoundaryFocus(
        ...
        this._rowElements[
            position === BoundaryPosition.TOP ? 1 : this._rowElements.length - 2
        ].focus();
    ...
    private _handleSelectionChange(): void {
        ...
        if (
            begin.node.compareDocumentPosition(end.node) &
        ...
        const toRowColumn = ({
            ...
            const rowElement: any =
                node instanceof Text ? node.parentNode : node;
            let row =
                parseInt(rowElement?.getAttribute("aria-posinset"), 10) - 1;
            if (isNaN(row)) {
        ...
        if (
            ...
            (beginRowColumn.row === endRowColumn.row &&
                beginRowColumn.column >= endRowColumn.column)
        ) {
