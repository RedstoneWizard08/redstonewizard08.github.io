...
export class CellColorResolver {
    ...
    public resolve(
        ...
        this.result.ext =
            cell.bg & BgFlags.HAS_EXTENDED ? cell.extended.ext : 0;
        ...
        if (
            ...
            $variantOffset =
                (x * deviceCellWidth) % (Math.round(lineWidth) * 2);
        }
        ...
        $isSelected = this._selectionRenderModel.isCellSelected(
            this._terminal,
            x,
            y
        );
        if ($isSelected) {
                ...
                if (this.result.fg & FgFlags.INVERSE) {
                        ...
                        case Attributes.CM_P256:
                            $bg =
                                this._themeService.colors.ansi[
                                    this.result.fg & Attributes.PCOLOR_MASK
                                ].rgba;
                            break;
                        case Attributes.CM_RGB:
                            $bg =
                                ((this.result.fg & Attributes.RGB_MASK) << 8) |
                                0xff;
                            break;
                    ...
                    switch (this.result.bg & Attributes.CM_MASK) {
                        ...
                        case Attributes.CM_P256:
                            $bg =
                                this._themeService.colors.ansi[
                                    this.result.bg & Attributes.PCOLOR_MASK
                                ].rgba;
                            break;
                        case Attributes.CM_RGB:
                            $bg =
                                ((this.result.bg & Attributes.RGB_MASK) << 8) |
                                0xff;
                            break;
            ...
            if ($colors.selectionForeground) {
                $fg =
                    ($colors.selectionForeground.rgba >> 8) &
                    Attributes.RGB_MASK;
                $hasFg = true;
            }
            ...
            if (treatGlyphAsBackgroundColor(cell.getCode())) {
                ...
                if (
                    this.result.fg & FgFlags.INVERSE &&
                    (this.result.bg & Attributes.CM_MASK) ===
                    ...
                    if (this.result.fg & FgFlags.INVERSE) {
                        switch (this.result.bg & Attributes.CM_MASK) {
                            ...
                            case Attributes.CM_RGB:
                                $fg =
                                    ((this.result.bg & Attributes.RGB_MASK) <<
                                        8) |
                                    0xff;
                                break;
                            // No need to consider default bg color here as it's not possible
                        ...
                        switch (this.result.fg & Attributes.CM_MASK) {
                            ...
                            case Attributes.CM_RGB:
                                $fg =
                                    ((this.result.fg & Attributes.RGB_MASK) <<
                                        8) |
                                    0xff;
                                break;
                            case Attributes.CM_DEFAULT:
        ...
        if ($hasBg) {
            if ($isSelected) {
                ...
                $bg =
                    (cell.bg & ~Attributes.RGB_MASK & ~BgFlags.DIM) |
                    $bg |
                    Attributes.CM_RGB;
            } else {
                // Non-RGB attributes from model + override + force RGB color mode
                $bg =
                    (cell.bg & ~Attributes.RGB_MASK) | $bg | Attributes.CM_RGB;
            }
        }
        if ($hasFg) {
            // Non-RGB attributes from model + force disable inverse + override + force RGB color mode
            $fg =
                (cell.fg & ~Attributes.RGB_MASK & ~FgFlags.INVERSE) |
                $fg |
                Attributes.CM_RGB;
        }
            ...
            if ($hasBg && !$hasFg) {
                ...
                if (
                    ...
                    $fg =
                        ...
                        (this.result.bg &
                            (Attributes.RGB_MASK | Attributes.CM_MASK));
                }
                $hasFg = true;
            }
            if (!$hasBg && $hasFg) {
                // Resolve bg color type (default color has a different meaning in fg vs bg)
                if (
                    (this.result.fg & Attributes.CM_MASK) ===
                    Attributes.CM_DEFAULT
                ) {
                    $bg =
                        (this.result.bg &
                            ~(Attributes.RGB_MASK | Attributes.CM_MASK)) |
                        (($colors.foreground.rgba >> 8) &
                            Attributes.RGB_MASK &
                            Attributes.RGB_MASK) |
                        Attributes.CM_RGB;
                } else {
