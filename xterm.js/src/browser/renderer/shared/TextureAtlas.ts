...
export class TextureAtlas implements ITextureAtlas {
    ...
    private _cacheMap: FourKeyMap<
        ...
        IRasterizedGlyph
    > = new FourKeyMap();
    private _cacheMapCombined: FourKeyMap<
        string,
        number,
        number,
        number,
    ...
    private _workBoundingBox: IBoundingBox = {
        top: 0,
        left: 0,
        bottom: 0,
        right: 0,
    ...
    private readonly _onAddTextureAtlasCanvas =
        new EventEmitter<HTMLCanvasElement>();
    public readonly onAddTextureAtlasCanvas =
        this._onAddTextureAtlasCanvas.event;
    private readonly _onRemoveTextureAtlasCanvas =
        new EventEmitter<HTMLCanvasElement>();
    public readonly onRemoveTextureAtlasCanvas =
        ...
        for (let i = 33; i < 126; i++) {
            queue.enqueue(() => {
                if (
                    ...
                    const rasterizedGlyph = this._drawToCache(
                        i,
                        DEFAULT_COLOR,
                        DEFAULT_COLOR,
                        DEFAULT_EXT
    ...
    public clearTexture(): void {
        if (
            this._pages[0].currentRow.x === 0 &&
            this._pages[0].currentRow.y === 0
        ) {
            return;
        }
    ...
    private _createNewPage(): AtlasPage {
        ...
        if (
            ...
            this._pages.length >= Math.max(4, TextureAtlas.maxAtlasPages)
        ) {
            // Find the set of the largest 4 images, below the maximum size, with the highest
            // percentages used
            const pagesBySize = this._pages
                .filter((e) => {
                    return (
                        e.canvas.width * 2 <=
                        (TextureAtlas.maxTextureSize ||
                            Constants.FORCED_MAX_TEXTURE_SIZE)
                    );
                })
                .sort((a, b) => {
            ...
            const sortedMergingPagesIndexes = mergingPages
    ...
    public getRasterizedGlyphCombinedChar(
        chars: string,
        bg: number,
        fg: number,
        ext: number,
    ...
    private _getForegroundColor(
        ...
        const minimumContrastColor = this._getMinimumContrastColor(
            bg,
            bgColorMode,
            bgColor,
            fg,
    ...
    private _getMinimumContrastColor(
        ...
        if (
            this._config.minimumContrastRatio === 1 ||
            excludeFromContrastRatioDemands
        ) {
            return undefined;
        }
        ...
        const bgRgba = this._resolveBackgroundRgba(
            bgColorMode,
            bgColor,
            inverse
        );
        const fgRgba = this._resolveForegroundRgba(
            fgColorMode,
            fgColor,
            inverse,
            bold
        ...
        // non-dim cells
        const result = rgba.ensureContrastRatio(
            bgRgba,
            fgRgba,
            this._config.minimumContrastRatio / (dim ? 2 : 1)
        );
    ...
    private _drawToCache(
        ...
        const chars =
            typeof codeOrChars === "number"
                ? String.fromCharCode(codeOrChars)
                : codeOrChars;
        ...
        const allowedWidth = Math.min(
            this._config.deviceCellWidth * Math.max(chars.length, 2) +
                TMP_CANVAS_GLYPH_PADDING * 2,
            this._textureSize
        );
        ...
        const allowedHeight = Math.min(
            this._config.deviceCellHeight + TMP_CANVAS_GLYPH_PADDING * 4,
            this._textureSize
        );
        ...
        const powerlineGlyph =
            chars.length === 1 && isPowerlineGlyph(chars.charCodeAt(0));
        const restrictedPowerlineGlyph =
            chars.length === 1 &&
            isRestrictedPowerlineGlyph(chars.charCodeAt(0));
        const foregroundColor = this._getForegroundColor(
            bg,
            bgColorMode,
            bgColor,
            fg,
        ...
        const padding = restrictedPowerlineGlyph
            ? 0
            : TMP_CANVAS_GLYPH_PADDING * 2;
        ...
        if (underline) {
            ...
            const lineWidth = Math.max(
                1,
                Math.floor(
                    (this._config.fontSize * this._config.devicePixelRatio) / 15
                )
            );
            ...
            const yTop =
                ...
                yOffset -
                (restrictToCellHeight ? lineWidth * 2 : 0);
                ...
                switch (this._workAttributeData.extended.underlineStyle) {
                    ...
                    case UnderlineStyle.DOTTED:
                        const offsetWidth =
                            nextOffset === 0
                                ? 0
                                : nextOffset >= lineWidth
                                  ? lineWidth * 2 - nextOffset
                                  : lineWidth - nextOffset;
                        // a line and a gap.
                        const isLineStart =
                            nextOffset >= lineWidth ? false : true;
                        if (isLineStart === false || offsetWidth === 0) {
                            ...
                            this._tmpCtx.moveTo(
                                xChLeft + offsetWidth + lineWidth,
                                yTop
                            );
                            this._tmpCtx.lineTo(xChRight, yTop);
                        }
                        nextOffset = computeNextVariantOffset(
                            xChRight - xChLeft,
                            lineWidth,
                            nextOffset
                        );
            ...
            if (!customGlyph && this._config.fontSize >= 12) {
                ...
                if (!this._config.allowTransparency && chars !== " ") {
                    ...
                    if (
                        ...
                        clipRegion.rect(
                            xLeft,
                            yTop - Math.ceil(lineWidth / 2),
                            this._config.deviceCellWidth * chWidth,
                            yBot - yTop + Math.ceil(lineWidth / 2)
                        );
                        this._tmpCtx.clip(clipRegion);
                        this._tmpCtx.lineWidth =
                            this._config.devicePixelRatio * 3;
                        this._tmpCtx.strokeStyle = backgroundColor.css;
                        this._tmpCtx.strokeText(
                            chars,
                            padding,
                            padding + this._config.deviceCharHeight
                        );
                        this._tmpCtx.restore();
                    }
                }
        ...
        if (overline) {
            ...
            this._tmpCtx.lineTo(
                padding + this._config.deviceCharWidth * chWidth,
                padding + yOffset
            );
            this._tmpCtx.stroke();
        }
        ...
        if (chars === "_" && !this._config.allowTransparency) {
            let isBeyondCellBounds = clearColor(
            ...
            if (isBeyondCellBounds) {
                for (let offset = 1; offset <= 5; offset++) {
                    ...
                    this._tmpCtx.fillText(
                        chars,
                        padding,
                        padding + this._config.deviceCharHeight - offset
                    );
                    isBeyondCellBounds = clearColor(
                        this._tmpCtx.getImageData(
                            padding,
                            padding,
                            this._config.deviceCellWidth,
        ...
        if (strikethrough) {
            const lineWidth = Math.max(
                1,
                Math.floor(
                    (this._config.fontSize * this._config.devicePixelRatio) / 10
                )
            );
        ...
        const rasterizedGlyph = this._findGlyphBoundingBox(
            imageData,
            this._workBoundingBox,
            allowedWidth,
            restrictedPowerlineGlyph,
            customGlyph,
            padding
        );
        ...
        while (true) {
            ...
            for (let i = this._activePages.length - 1; i >= 0; i--) {
                for (const row of this._activePages[i].fixedRows) {
                    if (
                        row.height <= activeRow.height &&
                        rasterizedGlyph.size.y <= row.height
                    ) {
            ...
            // fixed height
            if (
                activeRow.y + rasterizedGlyph.size.y >=
                    activePage.canvas.height ||
                activeRow.height >
                    rasterizedGlyph.size.y + Constants.ROW_PIXEL_THRESHOLD
            ) {
                // Create the new fixed height row, creating a new page if there isn't enough room on the
                // current page
                let wasPageAndRowFound = false;
                if (
                    activePage.currentRow.y +
                        activePage.currentRow.height +
            ...
            if (activeRow === activePage.currentRow) {
                ...
                activePage.fixedRows.splice(
                    activePage.fixedRows.indexOf(activeRow),
                    1
                );
            }
        }
        ...
        rasterizedGlyph.texturePositionClipSpace.x =
            activeRow.x / activePage.canvas.width;
        rasterizedGlyph.texturePositionClipSpace.y =
            activeRow.y / activePage.canvas.height;
    ...
    private _findGlyphBoundingBox(
        ...
        allowedWidth: number,
        restrictedGlyph: boolean,
        customGlyph: boolean,
        padding: number
    ): IRasterizedGlyph {
        boundingBox.top = 0;
        const height = restrictedGlyph
            ? this._config.deviceCellHeight
            : this._tmpCanvas.height;
        const width = restrictedGlyph
            ? this._config.deviceCellWidth
            : allowedWidth;
        let found = false;
...
class AtlasPage {
    ...
    constructor(document: Document, size: number, sourcePages?: AtlasPage[]) {
        if (sourcePages) {
...
function clearColor(
    ...
    const threshold = Math.floor(
        (Math.abs(r - fgR) + Math.abs(g - fgG) + Math.abs(b - fgB)) / 12
    );
        ...
        if (
            ...
            if (
                ...
                Math.abs(imageData.data[offset] - r) +
                    ...
                    Math.abs(imageData.data[offset + 2] - b) <
                    threshold
            ) {
                imageData.data[offset + 3] = 0;
            } else {
