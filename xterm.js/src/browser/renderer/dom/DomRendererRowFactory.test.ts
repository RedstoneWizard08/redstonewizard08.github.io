...
const EMPTY_WIDTH = new TestWidthCache(
    dom.window.document,
...
describe("DomRendererRowFactory", () => {
    ...
    describe("createRow", () => {
        ...
        it("should set correct attributes for double width characters", () => {
            ...
            lineData.setCell(
                1,
                CellData.fromCharData([DEFAULT_ATTR, "", 0, 0])
            );
            const spans = rowFactory.createRow(
                lineData,
                0,
            ...
            assert.equal(extractHtml(spans), "<span>語</span>");
        });
        ...
        it("should not display cursor for before initializing", () => {
            ...
            assert.equal(extractHtml(spans), `<span> </span>`);
        });
    ...
    describe("createRow with merged spans", () => {
        ...
        it("can merge codepoints for equal spacing", () => {
            lineData.setCell(
                0,
                CellData.fromCharData([DEFAULT_ATTR, "a", 1, "a".charCodeAt(0)])
            );
            lineData.setCell(
                1,
                CellData.fromCharData([DEFAULT_ATTR, "b", 1, "b".charCodeAt(0)])
            );
            lineData.setCell(
                2,
                CellData.fromCharData([DEFAULT_ATTR, "c", 1, "c".charCodeAt(0)])
            );
            const spans = rowFactory.createRow(
                lineData,
                0,
            ...
            assert.equal(extractHtml(spans), "<span>abc</span>");
        });
        ...
        it("should not merge codepoints with different spacing", () => {
            ...
            lineData.setCell(
                1,
                CellData.fromCharData([DEFAULT_ATTR, "€", 1, "€".charCodeAt(0)])
            );
            lineData.setCell(
                2,
                CellData.fromCharData([DEFAULT_ATTR, "c", 1, "c".charCodeAt(0)])
        ...
        it("should not merge on FG change", () => {
            const aColor1 = CellData.fromCharData([
                DEFAULT_ATTR,
                "a",
                1,
                "a".charCodeAt(0),
            ...
            const bColor2 = CellData.fromCharData([
                DEFAULT_ATTR,
                "b",
                1,
                "b".charCodeAt(0),
        ...
        it("should not merge cursor cell", () => {
            ...
            lineData.setCell(
                2,
                CellData.fromCharData([DEFAULT_ATTR, "X", 1, "X".charCodeAt(0)])
            );
            lineData.setCell(
                3,
                CellData.fromCharData([DEFAULT_ATTR, "b", 1, "b".charCodeAt(0)])
        ...
        it("should apply correct positive or negative spacing", () => {
            ...
            lineData.setCell(
                4,
                CellData.fromCharData([DEFAULT_ATTR, "𝄞", 1, "c".charCodeAt(0)])
            );
            const spans = rowFactory.createRow(
                lineData,
                0,
