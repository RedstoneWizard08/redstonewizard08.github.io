...
export class BufferLine implements IBufferLine {
    ...
    public set(index: number, value: CharData): void {
        ...
        if (value[CHAR_DATA_CHAR_INDEX].length > 1) {
            ...
            this._data[index * CELL_SIZE + Cell.CONTENT] =
                index |
    ...
    public hasWidth(index: number): number {
        return (
            this._data[index * CELL_SIZE + Cell.CONTENT] & Content.WIDTH_MASK
        );
    }
    ...
    public addCodepointToCell(
        ...
        if (content & Content.IS_COMBINED_MASK) {
            ...
            if (content & Content.CODEPOINT_MASK) {
                ...
                //  - set combined flag
                this._combined[index] =
                    stringFromCodePoint(content & Content.CODEPOINT_MASK) +
    ...
    public replaceCells(
        ...
        if (respectProtect) {
            if (
                start &&
                this.getWidth(start - 1) === 2 &&
                !this.isProtected(start - 1)
            ) {
                this.setCellFromCodepoint(start - 1, 0, 1, fillCellData);
            }
            if (
                end < this.length &&
                this.getWidth(end - 1) === 2 &&
                !this.isProtected(end)
            ) {
    ...
    public resize(cols: number, fillCellData: ICellData): boolean {
        if (cols === this.length) {
            return (
                this._data.length * 4 * CLEANUP_THRESHOLD <
        ...
        this.length = cols;
        return (
            uint32Cells * 4 * CLEANUP_THRESHOLD < this._data.buffer.byteLength
        );
    }
    ...
    public getTrimmedLength(): number {
        for (let i = this.length - 1; i >= 0; --i) {
            if (
                this._data[i * CELL_SIZE + Cell.CONTENT] &
                Content.HAS_CONTENT_MASK
            ) {
                return (
                    i +
    ...
    public getNoBgTrimmedLength(): number {
        for (let i = this.length - 1; i >= 0; --i) {
            if (
                this._data[i * CELL_SIZE + Cell.CONTENT] &
                    Content.HAS_CONTENT_MASK ||
                this._data[i * CELL_SIZE + Cell.BG] & Attributes.CM_MASK
            ) {
                return (
                    i +
                    (this._data[i * CELL_SIZE + Cell.CONTENT] >>
    ...
    public copyCellsFrom(
        ...
        if (applyInReverse) {
            for (let cell = length - 1; cell >= 0; cell--) {
                ...
                if (
                    srcData[(srcCol + cell) * CELL_SIZE + Cell.BG] &
                    BgFlags.HAS_EXTENDED
                ) {
                    this._extendedAttrs[destCol + cell] =
                        src._extendedAttrs[srcCol + cell];
                }
            }
        } else {
