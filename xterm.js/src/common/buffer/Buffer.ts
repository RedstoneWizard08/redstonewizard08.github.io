...
export class Buffer implements IBuffer {
    ...
    constructor(
        ...
        this.lines = new CircularList<IBufferLine>(
    ...
    private _getCorrectBufferLength(rows: number): number {
        ...
        const correctBufferLength =
            rows + this._optionsService.rawOptions.scrollback;

        return correctBufferLength > MAX_BUFFER_SIZE
            ? MAX_BUFFER_SIZE
            : correctBufferLength;
    }
        ...
        if (this.lines.length > 0) {
            ...
            if (this._cols < newCols) {
                for (let i = 0; i < this.lines.length; i++) {
                    ...
                    dirtyMemoryLines += +this.lines
                        .get(i)!
                        .resize(newCols, nullCell);
                }
            }
                    ...
                    if (this.lines.length < newRows + this.ybase) {
                        if (
                            ...
                            if (
                                ...
                                this.lines.push(
                                    new BufferLine(newCols, nullCell)
                                );
                            }
                        }
                    }
    ...
    private _reflowLarger(newCols: number, newRows: number): void {
        ...
        if (toRemove.length > 0) {
            const newLayoutResult = reflowLargerCreateNewLayout(
                this.lines,
                toRemove
            );
    ...
    private _reflowSmaller(newCols: number, newRows: number): void {
        ...
        for (let y = this.lines.length - 1; y >= 0; y--) {
            ...
            const lastLineLength =
                wrappedLines[wrappedLines.length - 1].getTrimmedLength();
            const destLineLengths = reflowSmallerGetNewLineLengths(
                wrappedLines,
                this._cols,
                newCols
            );
            ...
            for (let i = 0; i < linesToAdd; i++) {
                const newLine = this.getBlankLine(
                    DEFAULT_ATTR_DATA,
                    true
                ) as BufferLine;
            ...
            this.savedY = Math.min(
                this.savedY + linesToAdd,
                this.ybase + newRows - 1
            );
        }
        ...
        if (toInsert.length > 0) {
            ...
            for (
                ...
                if (
                    ...
                    for (
                        let nextI = nextToInsert.newLines.length - 1;
                        nextI >= 0;
                        nextI--
                    ) {
    ...
    public setupTabStops(i?: number): void {
        ...
        for (
            ;
            i < this._cols;
            i += this._optionsService.rawOptions.tabStopWidth
        ) {
            this.tabs[i] = true;
        }
    }
