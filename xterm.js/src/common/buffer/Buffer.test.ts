...
describe("Buffer", () => {
    ...
    describe("constructor", () => {
        it("should create a CircularList with max length equal to rows + scrollback, for its lines", () => {
            ...
            assert.equal(
                ...
                bufferService.rows + INIT_SCROLLBACK
            );
        });
    ...
    describe("getWrappedRangeForLine", () => {
        describe("non-wrapped", () => {
            it("should return a single row for the first row", () => {
                buffer.fillViewportRows();
                assert.deepEqual(buffer.getWrappedRangeForLine(0), {
                    first: 0,
                    last: 0,
                });
            });
            it("should return a single row for a middle row", () => {
                buffer.fillViewportRows();
                assert.deepEqual(buffer.getWrappedRangeForLine(12), {
                    first: 12,
                    last: 12,
                });
            });
            it("should return a single row for the last row", () => {
                buffer.fillViewportRows();
                assert.deepEqual(
                    buffer.getWrappedRangeForLine(buffer.lines.length - 1),
                    { first: 23, last: 23 }
                );
            });
        });
        describe("wrapped", () => {
            ...
            it("should return a range for a row that wraps upward to first row", () => {
                buffer.fillViewportRows();
                buffer.lines.get(1)!.isWrapped = true;
                assert.deepEqual(buffer.getWrappedRangeForLine(1), {
                    first: 0,
                    last: 1,
                });
            });
            it("should return a range for a row that wraps downward to last row", () => {
                ...
                assert.deepEqual(
                    buffer.getWrappedRangeForLine(buffer.lines.length - 2),
                    { first: 22, last: 23 }
                );
            });
        });
    ...
    describe("resize", () => {
        ...
        describe("row size reduced", () => {
            ...
            describe("no scrollback", () => {
                it("should trim from the top of the buffer when the cursor reaches the bottom", () => {
                    ...
                    buffer.lines
                        .get(5)!
                        .setCell(0, CellData.fromCharData(chData));
                    chData = buffer.lines
                        .get(INIT_ROWS - 1)!
                        .loadCell(0, new CellData())
                        .getAsCharData();
        ...
        describe("row size increased", () => {
            ...
            describe("filled buffer", () => {
                it("should show more of the buffer above", () => {
                    ...
                    for (let i = 0; i < 10; i++) {
                        buffer.lines.push(
                            buffer.getBlankLine(DEFAULT_ATTR_DATA)
                        );
                    }
        ...
        describe("reflow", () => {
            ...
            it("should discard parts of wrapped lines that go out of the scrollback", () => {
                ...
                assert.equal(buffer.lines.length, 5);
                assert.equal(
                    buffer.lines.get(0)!.translateToString(),
                    "fghij     "
                );
                assert.equal(
                    buffer.lines.get(1)!.translateToString(),
                    "          "
                );
                assert.equal(
                    buffer.lines.get(2)!.translateToString(),
                    "          "
                );
                assert.equal(
                    buffer.lines.get(3)!.translateToString(),
                    "          "
                );
                assert.equal(
                    buffer.lines.get(4)!.translateToString(),
                    "          "
                );
            });
            it("should remove the correct amount of rows when reflowing larger", () => {
                ...
                for (let i = 2; i < 10; i++) {
                    assert.equal(
                        buffer.lines.get(i)!.translateToString(),
                        "          "
                    );
                }
                buffer.resize(2, 10);
            ...
            it("should adjust markers when reflowing", () => {
                ...
                assert.equal(
                    firstMarker.line,
                    0,
                    "first marker should remain unchanged"
                );
                assert.equal(
                    secondMarker.line,
                    5,
                ...
                assert.equal(
                    secondMarker.line,
                    1,
                    "second marker should be restored to it's original line"
                );
                assert.equal(
                    thirdMarker.line,
                    2,
                    "third marker should be restored to it's original line"
                );
            ...
            it("should dispose markers whose rows are trimmed during a reflow", () => {
                ...
                assert.equal(
                    secondMarker.line,
                    1,
                    "second marker should be restored"
                );
                assert.equal(
                    thirdMarker.line,
                    2,
                    "third marker should be restored"
                );
            });
            it("should correctly reflow wrapped lines that end in 0 space (via tab char)", () => {
                ...
                assert.equal(buffer.lines.length, 10);
                assert.equal(
                    buffer.lines.get(0)!.translateToString(true),
                    "ab  c"
                );
                assert.equal(
                    buffer.lines.get(1)!.translateToString(false),
                    "d    "
                );
                buffer.resize(6, 10);
                assert.equal(buffer.ybase, 0);
            ...
            it("should wrap wide characters correctly when reflowing larger", () => {
                ...
                assert.equal(
                    buffer.lines.get(1)!.translateToString(true),
                    "汉语汉语汉语"
                );
                buffer.resize(13, 10);
                assert.equal(buffer.ybase, 0);
                ...
                assert.equal(
                    buffer.lines.get(0)!.translateToString(false),
                    "汉语汉语汉语 "
                );
                assert.equal(
                    buffer.lines.get(1)!.translateToString(true),
            ...
            it("should correctly reflow wrapped lines that end in 0 space (via tab char)", () => {
                ...
                assert.equal(
                    buffer.lines.get(2)!.translateToString(false),
                    "cd"
                );
            });
            it("should wrap wide characters correctly when reflowing smaller", () => {
                ...
                assert.equal(
                    buffer.lines.get(2)!.translateToString(true),
                    "汉语"
                );
                buffer.resize(10, 10);
                assert.equal(
                ...
                assert.equal(
                    buffer.lines.get(3)!.translateToString(true),
                    "语汉语"
                );
                buffer.resize(6, 10);
                assert.equal(
                    ...
                    describe("ybase !== 0", () => {
                        ...
                        describe("&& ydisp === ybase", () => {
                            it("should adjust the viewport and keep ydisp = ybase", () => {
                                ...
                                assert.equal(
                                    buffer.lines.get(10)!.translateToString(),
                                    "abcd"
                                );
                                assert.equal(
                                    buffer.lines.get(11)!.translateToString(),
                                    "efgh"
                                );
                                assert.equal(
                                    buffer.lines.get(12)!.translateToString(),
                                    "ijkl"
                                );
                                for (let i = 13; i < 17; i++) {
                                    assert.equal(
            ...
            describe("reflowSmaller cases", () => {
                ...
                describe("viewport not yet filled", () => {
                    it("should move the cursor down", () => {
                        ...
                        assert.equal(
                            buffer.lines.get(5)!.translateToString(),
                            "kl"
                        );
                        for (let i = 6; i < 10; i++) {
                            assert.equal(
                    ...
                    describe("ybase !== 0", () => {
                        ...
                        describe("&& ydisp === ybase", () => {
                            it("should adjust the viewport and keep ydisp = ybase", () => {
                                ...
                                assert.equal(
                                    buffer.lines.get(13)!.translateToString(),
                                    "gh"
                                );
                                assert.equal(
                                    buffer.lines.get(14)!.translateToString(),
                                    "ij"
                                );
                                assert.equal(
                                    buffer.lines.get(15)!.translateToString(),
                                    "kl"
                                );
                                for (let i = 16; i < 23; i++) {
                                    assert.equal(
                    ...
                    describe("ybase !== 0", () => {
                        ...
                        describe("&& ydisp === ybase", () => {
                            it("should trim lines and keep ydisp = ybase", () => {
                                ...
                                assert.equal(
                                    buffer.lines.get(7)!.translateToString(),
                                    "ab"
                                );
                                assert.equal(
                                    buffer.lines.get(8)!.translateToString(),
                                    "cd"
                                );
                                assert.equal(
                                    buffer.lines.get(9)!.translateToString(),
                                    "ef"
                                );
                                assert.equal(
                                    buffer.lines.get(10)!.translateToString(),
    ...
    describe("translateBufferLineToString", () => {
        it("should handle selecting a section of ascii text", () => {
            ...
            line.setCell(
                0,
                CellData.fromCharData([0, "a", 1, "a".charCodeAt(0)])
            );
            line.setCell(
                1,
                CellData.fromCharData([0, "b", 1, "b".charCodeAt(0)])
            );
            line.setCell(
                2,
                CellData.fromCharData([0, "c", 1, "c".charCodeAt(0)])
            );
            line.setCell(
                3,
                CellData.fromCharData([0, "d", 1, "d".charCodeAt(0)])
            );
        ...
        it("should handle a zero width character in the middle of the string by not including it", () => {
            ...
            line.setCell(
                0,
                CellData.fromCharData([0, "語", 2, "語".charCodeAt(0)])
            );
            line.setCell(1, CellData.fromCharData([0, "", 0, 0]));
            line.setCell(
                2,
        ...
        it("should handle single width emojis", () => {
            ...
            line.setCell(
                0,
                CellData.fromCharData([0, "😁", 1, "😁".charCodeAt(0)])
            );
            line.setCell(
                1,
                CellData.fromCharData([0, "a", 1, "a".charCodeAt(0)])
        ...
        it("should handle double width emojis", () => {
            ...
            line.setCell(
                0,
                CellData.fromCharData([0, "😁", 2, "😁".charCodeAt(0)])
            );
            line.setCell(1, CellData.fromCharData([0, "", 0, 0]));
            buffer.lines.set(0, line);
    ...
    describe("memory cleanup after shrinking", () => {
        it("should realign memory from idle task execution", async () => {
            ...
            for (let i = 0; i < INIT_ROWS; i++) {
                ...
                assert.equal(
                    (line as any)._data.buffer.byteLength,
                    INIT_COLS * 3 * 4
                );
                // array.length and .length get immediately adjusted
                assert.equal(
                    (line as any)._data.length,
                    (INIT_COLS / 2 - 1) * 3
                );
