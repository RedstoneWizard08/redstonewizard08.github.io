...
export function reflowLargerGetLinesToRemove(
    ...
    for (let y = 0; y < lines.length - 1; y++) {
        ...
        let destLineIndex = 0;
        let destCol = getWrappedLineTrimmedLength(
            wrappedLines,
            destLineIndex,
            oldCols
        );
        ...
        while (srcLineIndex < wrappedLines.length) {
            const srcTrimmedTineLength = getWrappedLineTrimmedLength(
                wrappedLines,
                srcLineIndex,
                oldCols
            );
...
export function reflowSmallerGetNewLineLengths(
    ...
    while (cellsAvailable < cellsNeeded) {
        ...
        srcCol += newCols;
        const oldTrimmedLength = getWrappedLineTrimmedLength(
            wrappedLines,
            srcLine,
            oldCols
        );
...
export function getWrappedLineTrimmedLength(
    lines: BufferLine[],
    i: number,
    cols: number
): number {
    ...
    // length]
    const endsInNull =
        !lines[i].hasContent(cols - 1) && lines[i].getWidth(cols - 1) === 1;
