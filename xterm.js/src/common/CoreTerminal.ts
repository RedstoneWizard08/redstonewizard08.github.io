...
import {
    ...
    MINIMUM_COLS,
    MINIMUM_ROWS,
} from "common/services/BufferService";
...
export abstract class CoreTerminal extends Disposable implements ICoreTerminal {
    ...
    private readonly _onResize = this.register(
        new EventEmitter<{ cols: number; rows: number }>()
    );
    ...
    constructor(options: Partial<ITerminalOptions>) {
        ...
        this._bufferService = this.register(
            this._instantiationService.createInstance(BufferService)
        );
        this._instantiationService.setService(
            IBufferService,
            this._bufferService
        ...
        this._logService = this.register(
            this._instantiationService.createInstance(LogService)
        );
        this._instantiationService.setService(ILogService, this._logService);
        this.coreService = this.register(
            this._instantiationService.createInstance(CoreService)
        );
        ...
        this._charsetService =
            this._instantiationService.createInstance(CharsetService);
        this._instantiationService.setService(
            ICharsetService,
            this._charsetService
        );
        this._oscLinkService =
            this._instantiationService.createInstance(OscLinkService);
        this._instantiationService.setService(
            IOscLinkService,
            this._oscLinkService
        );
        ...
        this.register(
            this._bufferService.onScroll((event) => {
                this._onScroll.fire({
        ...
        this.register(
            this._inputHandler.onScroll((event) => {
                this._onScroll.fire({
                    position: this._bufferService.buffer.ydisp,
                    source: ScrollSource.TERMINAL,
                });
        ...
        this._writeBuffer = this.register(
            new WriteBuffer((data, promiseResult) =>
                this._inputHandler.parse(data, promiseResult)
            )
        );
        this.register(
            forwardEvent(this._writeBuffer.onWriteParsed, this._onWriteParsed)
        );
    }
    ...
    public writeSync(
        ...
        maxSubsequentCalls?: number
    ): void {
        if (
            this._logService.logLevel <= LogLevelEnum.WARN &&
            !hasWriteSyncWarnHappened
        ) {
    ...
    protected _enableWindowsWrappingHeuristics(): void {
        if (!this._windowsWrappingHeuristics.value) {
            ...
            disposables.push(
                this.onLineFeed(
                    updateWindowsModeWrappedState.bind(
                        null,
                        this._bufferService
                    )
                )
