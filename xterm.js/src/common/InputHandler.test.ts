...
import {
    ...
    MockOptionsService,
    MockLogService,
    MockCoreMouseService,
    MockCharsetService,
    MockUnicodeService,
    MockOscLinkService,
} from "common/TestUtils.test";
...
function getLines(
    ...
    limit: number = bufferService.rows
): string[] {
    const res: string[] = [];
...
describe("InputHandler", () => {
    ...
    describe("SL/SR/DECIC/DECDC", () => {
        ...
        it("SL (scrollLeft)", async () => {
            ...
            assert.deepEqual(getLines(bufferService, 6), [
                "12345",
                "2345",
                "2345",
                "2345",
    ...
    describe("regression tests", function (): void {
        ...
        it("insertChars", async () => {
            ...
            assert.equal(
                ...
                Array(bufferService.cols - 9).join("a") + "1234567890"
            );
            ...
            assert.equal(
                line1.translateToString(false),
                Array(bufferService.cols - 9).join("a") + " 123456789"
            );
            ...
            assert.equal(
                line1.translateToString(false),
                Array(bufferService.cols - 9).join("a") + "  12345678"
            );
            ...
            assert.equal(
                line1.translateToString(false),
                Array(bufferService.cols - 9).join("a") + "    123456"
            );
            ...
            assert.equal(
                line1.translateToString(false),
                Array(bufferService.cols - 9).join("a") + "          "
            );
            assert.equal(
        ...
        it("deleteChars", async () => {
            ...
            assert.equal(
                line1.translateToString(false),
                Array(bufferService.cols - 9).join("a") + "234567890 "
            );
            assert.equal(
                line1.translateToString(true),
                Array(bufferService.cols - 9).join("a") + "234567890"
            ...
            assert.equal(
                line1.translateToString(false),
                Array(bufferService.cols - 9).join("a") + "34567890  "
            );
            assert.equal(
                line1.translateToString(true),
                Array(bufferService.cols - 9).join("a") + "34567890"
            ...
            assert.equal(
                line1.translateToString(false),
                Array(bufferService.cols - 9).join("a") + "567890    "
            );
            assert.equal(
                line1.translateToString(true),
                Array(bufferService.cols - 9).join("a") + "567890"
        ...
        it("eraseInLine reflow", async () => {
            ...
            const resetToBaseState = async (): Promise<void> => {
                ...
                await inputHandler.parseP(
                    Array(bufferService.cols + 10).join("a")
                ); // line 1 and 2
                ...
                assert.equal(
                    bufferService.buffer.lines.get(2)!.isWrapped,
                    true
                );
            };
        ...
        it("eraseInDisplay", async () => {
            ...
            assert.deepEqual(termContent(bufferService, false), [
                ...
                Array(bufferService.cols + 1).join("a"),
                Array(40 + 1).join("a") +
                    Array(bufferService.cols - 40 + 1).join(" "),
                Array(bufferService.cols + 1).join(" "),
            ]);
            ...
            assert.deepEqual(termContent(bufferService, false), [
                ...
                Array(bufferService.cols + 1).join(" "),
                Array(41 + 1).join(" ") +
                    Array(bufferService.cols - 41 + 1).join("a"),
                Array(bufferService.cols + 1).join("a"),
            ]);
            assert.deepEqual(termContent(bufferService, true), [
                "",
    ...
    describe("alt screen", () => {
        ...
        it("should handle DECSET/DECRST 1048 (alt screen cursor)", async () => {
            ...
            assert.equal(
                bufferService.buffer.lines.get(0)!.loadCell(0, new CellData())
                    .fg,
    ...
    describe("text attributes", () => {
        ...
        it("colormode palette 16", async () => {
            ...
            for (let i = 0; i < 8; ++i) {
                ...
                assert.equal(
                    inputHandler.curAttrData.getBgColorMode(),
                    Attributes.CM_P16
                );
    ...
    describe("colon notation", () => {
        ...
        describe("should equal to semicolon", () => {
            it("CSI 38:2::50:100:150 m", async () => {
                ...
                assert.equal(
                    inputHandler.curAttrData.fg,
                    inputHandler2.curAttrData.fg
                );
            });
    ...
    it("should parse big chunks in smaller subchunks", async () => {
        ...
        const calls: any[] = [];
        bufferService.resize(10, 10);
        (inputHandler as any)._parser.parse = (
            data: Uint32Array,
            length: number
        ) => {
    ...
    describe("BS with reverseWraparound set/unset", () => {
        ...
        describe("reverseWraparound set", () => {
            ...
            it("should lift isWrapped", async () => {
                ...
                await inputHandler.parseP("12345".repeat(2));
                assert.equal(
                    bufferService.buffer.lines.get(1)?.isWrapped,
                    true
                );
                await inputHandler.parseP(ttyBS.repeat(7));
                assert.equal(
            ...
            it("stops at hard NLs", async () => {
                ...
                assert.deepEqual(getLines(bufferService, 3), [
                    "12345",
                    "     ",
                    "     ",
                ]);
    ...
    describe("reset text attributes (SGR 0)", () => {
        ...
        it("resets all attributes except for the url", async () => {
            ...
            assert.deepEqual(
                inputHandler.curAttrData.extended,
                expectedExtended
            );
        });
    });
    ...
    describe("underline colors (SGR 58 & SGR 59)", () => {
        ...
        it("P256/RGB persistence", async () => {
            ...
            assert.equal(
                (bufferService.buffer!.lines.get(0)! as any)._extendedAttrs[2],
                undefined
            );
    ...
    describe("OSC", () => {
        ...
        it("4: set color events", async () => {
            ...
            assert.deepEqual(stack, [
                [{ type: ColorRequestType.SET, index: 0, color: [1, 2, 3] }],
            ]);
            stack.length = 0;
        ...
        describe("following sequence keeps working", () => {
            ...
            it("cursor never advances beyond cols", async () => {
                for (const seq of SEQ) {
                    ...
                    assert.equal(
                        bufferService.buffer.x <= bufferService.cols,
                        true
                    );
                    inputHandler.reset();
                    bufferService.reset();
    ...
    describe("DECRQM", () => {
        ...
        it("DEC privates with set/reset semantic", async () => {
            // initially reset
            const reset = [
                1, 6, 9, 12, 45, 66, 1000, 1002, 1003, 1004, 1006, 1016, 47,
                1047, 1049, 2004,
            ];
...
describe("InputHandler - async handlers", () => {
    ...
    it("async DCS between", async () => {
        inputHandler.registerDcsHandler(
            { final: "a" },
            async (data, params) => {
                await new Promise((res) => setTimeout(res, 50));
                assert.deepEqual(getLines(bufferService, 2), [
