...
export class OptionsService extends Disposable implements IOptionsService {
    ...
    private readonly _onOptionChange = this.register(
        new EventEmitter<keyof ITerminalOptions>()
    );
        ...
        for (const key in options) {
            if (key in defaultOptions) {
                try {
                    ...
                    defaultOptions[key] = this._sanitizeAndValidateOption(
                        key,
                        newValue
                    );
                } catch (e) {
    ...
    private _sanitizeAndValidateOption(key: string, value: any): any {
        switch (key) {
            ...
            case "fontWeightBold":
                ...
                value = FONT_WEIGHT_OPTIONS.includes(value)
                    ? value
                    : DEFAULT_OPTIONS[key];
                break;
            case "cursorWidth":
