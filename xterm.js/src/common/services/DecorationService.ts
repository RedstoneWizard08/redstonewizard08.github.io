    ...
    implements IDecorationService
{
    ...
    private readonly _decorations: SortedList<IInternalDecoration> =
        new SortedList((e) => e?.marker.line);

    private readonly _onDecorationRegistered = this.register(
        new EventEmitter<IInternalDecoration>()
    );
    public readonly onDecorationRegistered = this._onDecorationRegistered.event;
    private readonly _onDecorationRemoved = this.register(
        new EventEmitter<IInternalDecoration>()
    );
    ...
    public *getDecorationsAtCell(
        ...
        for (const d of this._decorations.getKeyIterator(line)) {
            ...
            if (
                x >= xmin &&
                x < xmax &&
                (!layer || (d.options.layer ?? "bottom") === layer)
            ) {
                yield d;
            }
        }
...
class Decoration extends Disposable implements IInternalDecoration {
    ...
    public readonly onRenderEmitter = this.register(
