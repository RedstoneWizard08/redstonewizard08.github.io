...
describe("Keyboard", () => {
    describe("evaluateKeyEscapeSequence", () => {
        it("should return the correct escape sequence for unmodified keys", () => {
            ...
            // Escape
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 27 }).key,
                "\x1b"
            );
            // Page up, page down
            assert.equal(
            ...
            // End, Home
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 35 }).key,
                "\x1b[F"
            ); // SS3 F
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 36 }).key,
                "\x1b[H"
            ); // SS3 H
            // Left, up, right, down arrows
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 37 }).key,
                "\x1b[D"
            ); // CSI D
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 38 }).key,
                "\x1b[A"
            ); // CSI A
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 39 }).key,
                "\x1b[C"
            ); // CSI C
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 40 }).key,
                "\x1b[B"
            ); // CSI B
            // Insert
            assert.equal(
            ...
            // F1-F12
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 112 }).key,
                "\x1bOP"
            ); // SS3 P
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 113 }).key,
                "\x1bOQ"
            ); // SS3 Q
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 114 }).key,
                "\x1bOR"
            ); // SS3 R
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 115 }).key,
                "\x1bOS"
            ); // SS3 S
            assert.equal(
        ...
        it("should handle lowercase letters", () => {
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 65, key: "a" }).key,
                "a"
            );
            assert.equal(
                testEvaluateKeyboardEvent({ keyCode: 189, key: "-" }).key,
                "-"
            );
        });
