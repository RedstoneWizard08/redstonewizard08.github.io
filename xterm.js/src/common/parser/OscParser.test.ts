    ...
    describe("handler registration", () => {
        ...
        it("addOscHandler with return false", () => {
            ...
            parser.registerHandler(
                1234,
                new TestHandler(1234, reports, "th2", true)
            );
            parser.start();
            let data = toUtf32("1234;Here comes");
            parser.put(data, 0, data.length);
    ...
    describe("sync and async mixed", () => {
        describe("sync | async | sync", () => {
            it("first should run, cleanup action for others", async () => {
                parser.registerHandler(
                    1234,
                    new TestHandler(1234, reports, "s1")
                );
                parser.registerHandler(
                    1234,
                    new TestHandlerAsync(1234, reports, "a1")
                );
                parser.registerHandler(
                    1234,
                    new TestHandler(1234, reports, "s2")
                );
                parser.start();
                let data = toUtf32("1234;Here comes");
                parser.put(data, 0, data.length);
            ...
            it("all should run", async () => {
                parser.registerHandler(
                    1234,
                    new TestHandler(1234, reports, "s1", true)
                );
                parser.registerHandler(
                    1234,
                    new TestHandlerAsync(1234, reports, "a1", true)
                );
                parser.registerHandler(
                    1234,
                    new TestHandler(1234, reports, "s2", true)
                );
                parser.start();
                let data = toUtf32("1234;Here comes");
                parser.put(data, 0, data.length);
        ...
        describe("async | sync | async", () => {
            it("first should run, cleanup action for others", async () => {
                parser.registerHandler(
                    1234,
                    new TestHandlerAsync(1234, reports, "s1")
                );
                parser.registerHandler(
                    1234,
                    new TestHandler(1234, reports, "a1")
                );
                parser.registerHandler(
                    1234,
                    new TestHandlerAsync(1234, reports, "s2")
                );
                parser.start();
                let data = toUtf32("1234;Here comes");
                parser.put(data, 0, data.length);
            ...
            it("all should run", async () => {
                parser.registerHandler(
                    1234,
                    new TestHandlerAsync(1234, reports, "s1", true)
                );
                parser.registerHandler(
                    1234,
                    new TestHandler(1234, reports, "a1", true)
                );
                parser.registerHandler(
                    1234,
                    new TestHandlerAsync(1234, reports, "s2", true)
                );
                parser.start();
                let data = toUtf32("1234;Here comes");
                parser.put(data, 0, data.length);
