...
export const VT500_TRANSITION_TABLE = (function (): TransitionTable {
    ...
    const blueprint = Array.apply(null, Array(BYTE_VALUES)).map(
        (unused: any, i: number) => i
    );
    const r = (start: number, end: number): number[] =>
        blueprint.slice(start, end);
    ...
    const states: number[] = r(
        ParserState.GROUND,
        ParserState.DCS_PASSTHROUGH + 1
    );
    let state: any;
...
export class EscapeSequenceParser
    extends Disposable
    implements IEscapeSequenceParser
{
    ...
    public setExecuteHandlerFallback(
        handler: ExecuteFallbackHandlerType
    ): void {
    ...
    public setCsiHandlerFallback(
        callback: (ident: number, params: IParams) => void
    ): void {
    ...
    public setErrorHandler(
        callback: (state: IParsingState) => IParsingState
    ): void {
            ...
            if (this._parseStack.state === ParserStackType.RESET) {
                ...
                switch (this._parseStack.state) {
                    case ParserStackType.CSI:
                        if (promiseResult === false && handlerPos > -1) {
                            for (; handlerPos >= 0; handlerPos--) {
                                handlerResult = (handlers as CsiHandlerType[])[
                                    handlerPos
                                ](this._params);
                    ...
                    case ParserStackType.ESC:
                        if (promiseResult === false && handlerPos > -1) {
                            for (; handlerPos >= 0; handlerPos--) {
                                handlerResult = (handlers as EscHandlerType[])[
                                    handlerPos
                                ]();
                                if (handlerResult === true) {
                                    break;
                    ...
                    case ParserStackType.DCS:
                        ...
                        handlerResult = this._dcsParser.unhook(
                            code !== 0x18 && code !== 0x1a,
                            promiseResult
                        );
                        if (handlerResult) {
                        ...
                        if (code === 0x1b)
                            this._parseStack.transition |= ParserState.ESCAPE;
                        this._params.reset();
                        this._params.addParam(0); // ZDM
                        this._collect = 0;
                        break;
                    case ParserStackType.OSC:
                        code = data[this._parseStack.chunkPos];
                        handlerResult = this._oscParser.end(
                            code !== 0x18 && code !== 0x1a,
                            promiseResult
                        );
                        if (handlerResult) {
                ...
                this.precedingJoinState = 0;
                this.currentState =
                    this._parseStack.transition &
                    TableAccess.TRANSITION_STATE_MASK;
            }
        }
        ...
        for (let i = start; i < length; ++i) {
            ...
            switch (transition >> TableAccess.TRANSITION_ACTION_SHIFT) {
                case ParserAction.PRINT:
                    ...
                    for (let j = i + 1; ; ++j) {
                        if (
                            j >= length ||
                            (code = data[j]) < 0x20 ||
                            (code > 0x7e && code < NON_ASCII_PRINTABLE)
                        ) {
                ...
                case ParserAction.CSI_DISPATCH:
                    // Trigger CSI Handler
                    const handlers =
                        this._csiHandlers[(this._collect << 8) | code];
                ...
                case ParserAction.ESC_DISPATCH:
                    const handlersEsc =
                        this._escHandlers[(this._collect << 8) | code];
