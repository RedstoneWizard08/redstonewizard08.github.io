...
} from "common/parser/EscapeSequenceParser";
    ...
    public end(success: boolean): void {
        ...
        if (!isNaN(id)) {
            this._fallback(
                id,
                "END",
                this.data.slice(this.data.indexOf(";") + 1)
            );
        }
    }
...
testParser.setOscHandlerFallback((identifier, action, data) => {
    if (identifier === -1)
        ...
        testTerminal.actionOSC("" + identifier + ";" + data); // collect only data at END
});
...
describe("EscapeSequenceParser", () => {
    ...
    describe("state transitions and actions", () => {
        ...
        it("trans ANYWHERE --> GROUND with actions", () => {
            ...
            const exceptions: { [key: number]: { [key: string]: any[] } } = {
                ...
                13: {
                    "\x18": [["dcs unhook", false]],
                    "\x1a": [["dcs unhook", false]],
    ...
    describe("set/clear handler", () => {
        ...
        const dcs: (
            | [string]
            | [string, string]
            | [string, string, ParamsArray, number]
        )[] = [];
        ...
        it("print handler", () => {
            parser2.setPrintHandler(function (
                data: Uint32Array,
                start: number,
                end: number
            ): void {
        ...
        describe("ESC custom handlers", () => {
            ...
            it("Dispose should work", () => {
                ...
                const dispo = parser2.registerEscHandler(
                    { intermediates: "%", final: "G" },
                    () => {
                        esc.push("custom - %G");
                        return true;
        ...
        it("CSI handler", () => {
            parser2.registerCsiHandler(
                { final: "m" },
                function (params: IParams): boolean {
        ...
        describe("CSI custom handlers", () => {
            it("Prevent fallback", () => {
                ...
                assert.deepEqual(
                    csi,
                    [],
                    "Should not fallback to original handler"
                );
                assert.deepEqual(csiCustom, [
                    ["m", [1, 31], ""],
                    ["m", [0], ""],
                ]);
            });
            ...
            it("Dispose should work", () => {
                ...
                const customHandler = parser2.registerCsiHandler(
                    { final: "m" },
                    (params) => {
                        csiCustom.push(["m", params.toArray(), ""]);
                        return true;
                ...
                assert.deepEqual(
                    csiCustom,
                    [],
                    "Should not use custom handler as it was disposed"
                );
            });
            it("Should not corrupt the parser when dispose is called twice", () => {
                const csiCustom: [string, ParamsArray, string][] = [];
        ...
        it("ERROR handler", () => {
            ...
            parser2.setErrorHandler(function (
                state: IParsingState
            ): IParsingState {
                errorState = state;
                return state;
    ...
    describe("function identifiers", () => {
        describe("registration limits", () => {
                ...
                for (let i = 0x3c; i <= 0x3f; ++i) {
                    ...
                    assert.equal(
                        parser.identToString(
                            parser.identifier({ prefix: c, final: "z" })
                        ),
                        c + "z"
                    );
                }
            ...
            it("final CSI/DCS range 0x40 .. 0x7e (default), one byte", () => {
                for (let i = 0x40; i <= 0x7e; ++i) {
                    const c = String.fromCharCode(i);
                    assert.equal(
                        parser.identToString(parser.identifier({ final: c })),
                        c
                    );
                }
                assert.throws(() => {
            ...
            it("id calculation - should stacking prefix -> intermediate -> final", () => {
                assert.equal(
                    parser.identToString(parser.identifier({ final: "z" })),
                    "z"
                );
                assert.equal(
                    parser.identToString(
        ...
        describe("identifier invocation", () => {
            it("ESC", () => {
                ...
                const h2 = parser.registerEscHandler(
                ...
                const h3 = parser.registerEscHandler(
                    { intermediates: "!!", final: "z" },
                    () => {
                        callstack.push("!!z");
                        return true;
            ...
            it("CSI", () => {
                const callstack: any[] = [];
                const h1 = parser.registerCsiHandler(
                    { final: "z" },
                    (params) => {
                ...
                const h2 = parser.registerCsiHandler(
                    { intermediates: "!", final: "z" },
                    (params) => {
                        callstack.push(["!z", params.toArray()]);
                        return true;
                ...
                const h3 = parser.registerCsiHandler(
                    { intermediates: "!!", final: "z" },
                    (params) => {
                        callstack.push(["!!z", params.toArray()]);
                        return true;
                ...
                const h4 = parser.registerCsiHandler(
                    { prefix: "?", final: "z" },
                    (params) => {
                        callstack.push(["?z", params.toArray()]);
                        return true;
...
describe("EscapeSequenceParser - async", () => {
    ...
    describe("async handlers", () => {
        ...
        it("multiple async SGR handlers", async () => {
            ...
            const SGR2 = parser.registerCsiHandler(
                { final: "m" },
                async (params) => {
            ...
            assert.deepEqual(
                callstack,
                RESULT,
                "Should not call custom handler"
            );
            evalStackSaves(parser.trackedStack, [
                [6, ParserStackType.CSI, 0],
                [15, ParserStackType.ESC, 0],
            ...
            const SGR22 = parser.registerCsiHandler(
                { final: "m" },
                async (params) => {
                    callstack.push(["2# SGR", params.toArray()]);
                    return true;
        ...
        it("multiple async ESC handlers", async () => {
            ...
            // register without fallback
            const ESC22 = parser.registerEscHandler(
                { final: "E" },
                async () => {
                    callstack.push(["2# ESC E"]);
                    return true;
        ...
        it("sync/async SGR mixed", async () => {
            ...
            // async with fallback
            const SGR3 = parser.registerCsiHandler(
                { final: "m" },
                async (params) => {
