...
export class Params implements IParams {
    ...
    public toArray(): ParamsArray {
        ...
        for (let i = 0; i < this.length; ++i) {
            ...
            if (end - start > 0) {
                res.push(
                    Array.prototype.slice.call(this._subParams, start, end)
                );
            }
        }
        return res;
    ...
    public hasSubParams(idx: number): boolean {
        return (
            (this._subParamsIdx[idx] & 0xff) - (this._subParamsIdx[idx] >> 8) >
            0
        );
    }
    ...
    public addDigit(value: number): void {
        ...
        store[length - 1] = ~cur
            ? Math.min(cur * 10 + value, MAX_VALUE)
            : value;
    }
}
