 ...
 */
const GLEVEL: { [key: string]: number } = {
    "(": 0,
    ")": 1,
    "*": 2,
    "+": 3,
...
export class InputHandler extends Disposable implements IInputHandler {
    ...
    private readonly _onRequestRefreshRows = this.register(
    ...
    private readonly _onRequestSendFocus = this.register(
        new EventEmitter<void>()
    );
    public readonly onRequestSendFocus = this._onRequestSendFocus.event;
    private readonly _onRequestSyncScrollBar = this.register(
        new EventEmitter<void>()
    );
    public readonly onRequestSyncScrollBar = this._onRequestSyncScrollBar.event;
    private readonly _onRequestWindowsOptionsReport = this.register(
        new EventEmitter<WindowsOptionsReportType>()
    );
    public readonly onRequestWindowsOptionsReport =
    ...
    constructor(
         ...
         * print handler
         */
        this._parser.setPrintHandler((data, start, end) =>
            this.print(data, start, end)
        );

        /**
         * CSI handler
         */
        this._parser.registerCsiHandler({ final: "@" }, (params) =>
        ...
        this._parser.registerCsiHandler({ final: "A" }, (params) =>
        ...
        this._parser.registerCsiHandler({ final: "D" }, (params) =>
            this.cursorBackward(params)
        );
        this._parser.registerCsiHandler({ final: "E" }, (params) =>
            this.cursorNextLine(params)
        );
        this._parser.registerCsiHandler({ final: "F" }, (params) =>
            this.cursorPrecedingLine(params)
        );
        this._parser.registerCsiHandler({ final: "G" }, (params) =>
            this.cursorCharAbsolute(params)
        );
        this._parser.registerCsiHandler({ final: "H" }, (params) =>
            this.cursorPosition(params)
        );
        this._parser.registerCsiHandler({ final: "I" }, (params) =>
            this.cursorForwardTab(params)
        );
        this._parser.registerCsiHandler({ final: "J" }, (params) =>
            this.eraseInDisplay(params, false)
        );
        this._parser.registerCsiHandler({ prefix: "?", final: "J" }, (params) =>
            this.eraseInDisplay(params, true)
        );
        this._parser.registerCsiHandler({ final: "K" }, (params) =>
            this.eraseInLine(params, false)
        );
        this._parser.registerCsiHandler({ prefix: "?", final: "K" }, (params) =>
            this.eraseInLine(params, true)
        );
        this._parser.registerCsiHandler({ final: "L" }, (params) =>
            this.insertLines(params)
        );
        this._parser.registerCsiHandler({ final: "M" }, (params) =>
            this.deleteLines(params)
        );
        this._parser.registerCsiHandler({ final: "P" }, (params) =>
            this.deleteChars(params)
        );
        this._parser.registerCsiHandler({ final: "S" }, (params) =>
            this.scrollUp(params)
        );
        this._parser.registerCsiHandler({ final: "T" }, (params) =>
            this.scrollDown(params)
        );
        this._parser.registerCsiHandler({ final: "X" }, (params) =>
            this.eraseChars(params)
        );
        this._parser.registerCsiHandler({ final: "Z" }, (params) =>
        ...
        this._parser.registerCsiHandler({ final: "n" }, (params) =>
        ...
        this._parser.registerCsiHandler({ final: "r" }, (params) =>
        ...
        this._parser.registerCsiHandler({ final: "t" }, (params) =>
            this.windowOptions(params)
        );
        this._parser.registerCsiHandler({ final: "u" }, (params) =>
        ...
        this._parser.registerOscHandler(
            1,
            new OscHandler((data) => this.setIconName(data))
        );
        //   2 - title
        this._parser.registerOscHandler(
            2,
            new OscHandler((data) => this.setTitle(data))
        );
        ...
        this._parser.registerOscHandler(
            8,
            new OscHandler((data) => this.setHyperlink(data))
        );
        //  10 - Change VT100 text foreground color to Pt.
        this._parser.registerOscHandler(
            10,
            new OscHandler((data) => this.setOrReportFgColor(data))
        );
        //  11 - Change VT100 text background color to Pt.
        this._parser.registerOscHandler(
            11,
            new OscHandler((data) => this.setOrReportBgColor(data))
        );
        ...
        this._parser.registerOscHandler(
            104,
            new OscHandler((data) => this.restoreIndexedColor(data))
        );
        ...
        this._parser.registerOscHandler(
            110,
            new OscHandler((data) => this.restoreFgColor(data))
        );
        // 111 - Reset VT100 text background color.
        this._parser.registerOscHandler(
            111,
            new OscHandler((data) => this.restoreBgColor(data))
        );
        // 112 - Reset text cursor color.
        this._parser.registerOscHandler(
            112,
            new OscHandler((data) => this.restoreCursorColor(data))
        );
        ...
        this._parser.registerDcsHandler(
            ...
            new DcsHandler((data, params) =>
                this.requestStatusString(data, params)
            )
        );
    }
    ...
    private _logSlowResolvingAsync(p: Promise<boolean>): void {
        ...
        if (this._logService.logLevel <= LogLevelEnum.WARN) {
            Promise.race([
                p,
                new Promise((res, rej) =>
                    setTimeout(() => rej("#SLOW_TIMEOUT"), SLOW_ASYNC_LIMIT)
                ),
            ]).catch((err) => {
                if (err !== "#SLOW_TIMEOUT") {
                    throw err;
                ...
                console.warn(
                    `async parser handler taking longer than ${SLOW_ASYNC_LIMIT} ms`
                );
            });
        }
    }
    ...
    public parse(
        ...
        if (this._parseBuffer.length < data.length) {
            if (this._parseBuffer.length < MAX_PARSEBUFFER_LENGTH) {
                this._parseBuffer = new Uint32Array(
                    Math.min(data.length, MAX_PARSEBUFFER_LENGTH)
                );
            }
        }
        ...
        if (data.length > MAX_PARSEBUFFER_LENGTH) {
            for (let i = start; i < data.length; i += MAX_PARSEBUFFER_LENGTH) {
                const end =
                    i + MAX_PARSEBUFFER_LENGTH < data.length
        ...
        if (
            this._activeBuffer.x !== cursorStartX ||
            this._activeBuffer.y !== cursorStartY
        ) {
        ...
        if (viewportStart < this._bufferService.rows) {
            this._onRequestRefreshRows.fire(
                Math.min(viewportStart, this._bufferService.rows - 1),
                Math.min(viewportEnd, this._bufferService.rows - 1)
            );
        }
    }
    ...
    public print(data: Uint32Array, start: number, end: number): void {
        ...
        let bufferRow = this._activeBuffer.lines.get(
        ...
        for (let pos = start; pos < end; ++pos) {
            ...
            if (shouldJoin && this._activeBuffer.x) {
                const offset = bufferRow.getWidth(this._activeBuffer.x - 1)
                    ? 1
                    : 2;
    ...
    public backspace(): boolean {
        ...
        if (this._activeBuffer.x > 0) {
            ...
            if (
                ...
                //   cell further back
                const line = this._activeBuffer.lines.get(
                    this._activeBuffer.ybase + this._activeBuffer.y
                )!;
                if (
                    line.hasWidth(this._activeBuffer.x) &&
                    !line.hasContent(this._activeBuffer.x)
                ) {
                    this._activeBuffer.x--;
    ...
    private _restrictCursor(
        maxCol: number = this._bufferService.cols - 1
    ): void {
    ...
    public insertLines(params: IParams): boolean {
        ...
        const scrollBottomRowsOffset =
            this._bufferService.rows - 1 - this._activeBuffer.scrollBottom;
    ...
    private _is(term: string): boolean {
        return (
            (this._optionsService.rawOptions.termName + "").indexOf(term) === 0
        );
    }
    ...
    private _extractColor(
        ...
        do {
            ...
            ++advance + pos < params.length &&
            advance + cSpace < accu.length
        );
        ...
        for (let i = 0; i < l; i++) {
            ...
            if (p >= 30 && p <= 37) {
                ...
                attr.fg |=
                    ...
                    (Attributes.PCOLOR_MASK | Attributes.RGB_MASK);
            } else if (p === 49) {
                // reset bg
                attr.bg &= ~(Attributes.CM_MASK | Attributes.RGB_MASK);
                attr.bg |=
                    DEFAULT_ATTR_DATA.bg &
                    (Attributes.PCOLOR_MASK | Attributes.RGB_MASK);
            ...
            case 14: // GetWinSizePixels, returns CSI 4 ; height ; width t
                if (second !== 2) {
                    this._onRequestWindowsOptionsReport.fire(
                        WindowsOptionsReportType.GET_WIN_SIZE_PIXELS
                    );
                }
                break;
            case 16: // GetCellSizePixels, returns CSI 6 ; height ; width t
                this._onRequestWindowsOptionsReport.fire(
                    WindowsOptionsReportType.GET_CELL_SIZE_PIXELS
                );
                break;
                ...
                this._onColor.fire([
                    {
                        ...
                        index: this._specialColors[offset],
                    },
                ]);
            } else {
    ...
    public reverseIndex(): boolean {
        ...
        if (this._activeBuffer.y === this._activeBuffer.scrollTop) {
            ...
            const scrollRegionHeight =
                this._activeBuffer.scrollBottom - this._activeBuffer.scrollTop;
    ...
    public screenAlignmentPattern(): boolean {
        ...
        for (let yOffset = 0; yOffset < this._bufferService.rows; ++yOffset) {
            const row =
                this._activeBuffer.ybase + this._activeBuffer.y + yOffset;
    ...
    public requestStatusString(data: string, params: IParams): boolean {
        ...
        if (data === " q")
            return f(
                `P1$r${STYLES[opts.cursorStyle] - (opts.cursorBlink ? 1 : 0)} q`
            );
        return f(`P0$r`);
    }
