...
import {
    ...
    before,
    beforeEach,
    ThroughputRuntimeCase,
} from "xterm-benchmark";
...
perfContext("Parser throughput - 50MB data", () => {
    ...
    perfContext("ESCAPE with collect - ESC % G", () => {
        before(() => {
            const data =
                "\x1b%G\x1b%G\x1b%G\x1b%G\x1b%G\x1b%G\x1b%G\x1b%G\x1b%G\x1b%G";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("CSI with collect - CSI ? p", () => {
        before(() => {
            const data =
                "\x1b[?p\x1b[?p\x1b[?p\x1b[?p\x1b[?p\x1b[?p\x1b[?p\x1b[?p\x1b[?p\x1b[?p";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("CSI with params (short) - CSI 1;2 m", () => {
        before(() => {
            const data =
                "\x1b[1;2m\x1b[1;2m\x1b[1;2m\x1b[1;2m\x1b[1;2m\x1b[1;2m\x1b[1;2m\x1b[1;2m\x1b[1;2m\x1b[1;2m";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("CSI with params (long) - CSI 1;2;3;4;5;6;7;8;9;0 m", () => {
        before(() => {
            const data =
                "\x1b[1;2;3;4;5;6;7;8;9;0m\x1b[1;2;3;4;5;6;7;8;9;0m\x1b[1;2;3;4;5;6;7;8;9;0m";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("OSC string interface (short seq) - OSC 0;hi ST", () => {
        before(() => {
            const data =
                "\x1b]0;hi\x1b\\\x1b]0;hi\x1b\\\x1b]0;hi\x1b\\\x1b]0;hi\x1b\\x1b]0;hi\x1b\\";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("OSC class interface (short seq) - OSC 0;hi ST", () => {
        before(() => {
            const data =
                "\x1b]1;hi\x1b\\\x1b]1;hi\x1b\\\x1b]1;hi\x1b\\\x1b]1;hi\x1b\\x1b]1;hi\x1b\\";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("OSC class interface (long seq) - OSC 0;<text> ST", () => {
        before(() => {
            const data =
                "\x1b]1;Lorem ipsum dolor sit amet, consetetur sadipscing elitr.\x1b\\";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("DCS string interface (short seq)", () => {
        before(() => {
            const data =
                "\x1bPphi\x1b\\\x1bPphi\x1b\\\x1bPphi\x1b\\\x1bPphi\x1b\\\x1bPphi\x1b\\";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("DCS class interface (short seq)", () => {
        before(() => {
            const data =
                "\x1bPqhi\x1b\\\x1bPqhi\x1b\\\x1bPqhi\x1b\\\x1bPqhi\x1b\\\x1bPqhi\x1b\\";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
    ...
    perfContext("DCS class interface (long seq)", () => {
        before(() => {
            const data =
                "\x1bPqLorem ipsum dolor sit amet, consetetur sadipscing elitr.\x1b\\";
            let content = "";
            while (content.length < SIZE) {
                content += data;
            }
