...
export function injectSharedRendererTests(
    ctx: ISharedRendererTestContext
): void {
    ...
    test.describe("colors", () => {
        test("foreground 0-15", async () => {
            ...
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 1, 1),
                [1, 2, 3, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 2, 1),
                [4, 5, 6, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 3, 1),
                [7, 8, 9, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 4, 1),
                [10, 11, 12, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 5, 1),
                [13, 14, 15, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 6, 1),
                [16, 17, 18, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 7, 1),
                [19, 20, 21, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 8, 1),
                [22, 23, 24, 255]
            );
        });
    });
    ...
    test("foreground 0-15 inverse", async () => {
        ...
        await ctx.value.proxy.write(
            `\x1b[7;30m \x1b[7;31m \x1b[7;32m \x1b[7;33m \x1b[7;34m \x1b[7;35m \x1b[7;36m \x1b[7;37m `
        );
        await pollFor(
            ctx.value.page,
            () => getCellColor(ctx.value, 1, 1),
    ...
    test("background 0-15 inverse", async () => {
        ...
        await ctx.value.proxy.write(
            `\x1b[7;40m■\x1b[7;41m■\x1b[7;42m■\x1b[7;43m■\x1b[7;44m■\x1b[7;45m■\x1b[7;46m■\x1b[7;47m■`
        );
        await pollFor(
            ctx.value.page,
            () => getCellColor(ctx.value, 1, 1),
    ...
    test("foreground 0-15 invisible", async () => {
        ...
        await ctx.value.proxy.write(
            `\x1b[8;30m \x1b[8;31m \x1b[8;32m \x1b[8;33m \x1b[8;34m \x1b[8;35m \x1b[8;36m \x1b[8;37m `
        );
        await pollFor(
            ctx.value.page,
            () => getCellColor(ctx.value, 1, 1),
    ...
    test("background 0-15 invisible", async () => {
        ...
        await ctx.value.proxy.write(
            `\x1b[8;40m■\x1b[8;41m■\x1b[8;42m■\x1b[8;43m■\x1b[8;44m■\x1b[8;45m■\x1b[8;46m■\x1b[8;47m■`
        );
        await pollFor(
            ctx.value.page,
            () => getCellColor(ctx.value, 1, 1),
    ...
    test("foreground 16-255", async () => {
        ...
        for (let y = 0; y < 240 / 16; y++) {
            for (let x = 0; x < 16; x++) {
                ...
                await pollFor(
                    ctx.value.page,
                    () => getCellColor(ctx.value, x + 1, y + 1),
                    [r, g, b, 255]
                );
            }
        }
    ...
    test.describe("minimumContrastRatio", async () => {
      ...
      `);
            ...
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 1, 2),
                [0x55, 0x57, 0x53, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 2, 2),
                [0xef, 0x29, 0x29, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 3, 2),
                [0x8a, 0xe2, 0x34, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 4, 2),
                [0xfc, 0xe9, 0x4f, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 5, 2),
                [0x72, 0x9f, 0xcf, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 6, 2),
                [0xad, 0x7f, 0xa8, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 7, 2),
                [0x34, 0xe2, 0xe2, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 8, 2),
                [0xee, 0xee, 0xec, 255]
            );
            ...
            await ctx.value.page.evaluate(
                `window.term.options.minimumContrastRatio = 10;`
            );
            frameDetails = undefined;
            await pollFor(
                ctx.value.page,
      ...
      `);
            ...
            await pollForApproximate(
                ctx.value.page,
                marginOfError,
                () => getCellColor(ctx.value, 1, 1),
                [
                    Math.floor((255 + 0x2e) / 2),
                    Math.floor((255 + 0x34) / 2),
                    Math.floor((255 + 0x36) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 2, 1),
                [
                    Math.floor((255 + 0xcc) / 2),
                    Math.floor((255 + 0x00) / 2),
                    Math.floor((255 + 0x00) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 3, 1),
                [
                    Math.floor((255 + 0x4e) / 2),
                    Math.floor((255 + 0x9a) / 2),
                    Math.floor((255 + 0x06) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 4, 1),
                [
                    Math.floor((255 + 0xc4) / 2),
                    Math.floor((255 + 0xa0) / 2),
                    Math.floor((255 + 0x00) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                [
                    Math.floor((255 + 0x34) / 2),
                    Math.floor((255 + 0x65) / 2),
                    Math.floor((255 + 0xa4) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 6, 1),
                [
                    Math.floor((255 + 0x75) / 2),
                    Math.floor((255 + 0x50) / 2),
                    Math.floor((255 + 0x7b) / 2),
                    255,
                ]
            );
            await pollForApproximate(
            ...
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 8, 1),
                [
                    Math.floor((255 + 0xd3) / 2),
                    Math.floor((255 + 0xd7) / 2),
                    Math.floor((255 + 0xcf) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 1, 2),
                [
                    Math.floor((255 + 0x55) / 2),
                    Math.floor((255 + 0x57) / 2),
                    Math.floor((255 + 0x53) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 2, 2),
                [
                    Math.floor((255 + 0xef) / 2),
                    Math.floor((255 + 0x29) / 2),
                    Math.floor((255 + 0x29) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 3, 2),
                [
                    Math.floor((255 + 0x8a) / 2),
                    Math.floor((255 + 0xe2) / 2),
                    Math.floor((255 + 0x34) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 4, 2),
                [
                    Math.floor((255 + 0xfc) / 2),
                    Math.floor((255 + 0xe9) / 2),
                    Math.floor((255 + 0x4f) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 5, 2),
                [
                    Math.floor((255 + 0x72) / 2),
                    Math.floor((255 + 0x9f) / 2),
                    Math.floor((255 + 0xcf) / 2),
                    255,
                ]
            );
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 6, 2),
                [
                    Math.floor((255 + 0xad) / 2),
                    Math.floor((255 + 0x7f) / 2),
                    Math.floor((255 + 0xa8) / 2),
                    255,
                ]
            );
            await pollForApproximate(
            ...
            await pollForApproximate(
                ...
                () => getCellColor(ctx.value, 8, 2),
                [
                    Math.floor((255 + 0xee) / 2),
                    Math.floor((255 + 0xee) / 2),
                    Math.floor((255 + 0xec) / 2),
                    255,
                ]
            );
            // Setting and check for minimum contrast values, note that these are not
    ...
    (ctx.skipCanvasExceptions ? test.describe.skip : test.describe)(
        "selectionBackground",
        async () => {
    ...
    (ctx.skipCanvasExceptions || ctx.skipDomExceptions
        ? test.describe.skip
        : test.describe)("selection blending", () => {
        test("background", async () => {
            ...
            await ctx.value.proxy.writeln(
                "\x1b[48:2:0:204:0:0m red truecolor bg\x1b[0m"
            );
            await ctx.value.proxy.selectAll();
            await pollFor(
                ctx.value.page,
            ...
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 1, 3),
                [230, 128, 128, 255]
            );
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 1, 4),
                [230, 128, 128, 255]
            );
        });
        test("powerline decorative symbols", async () => {
            ...
            await ctx.value.proxy.writeln(
                "\x1b[31;42m\u{E0B4} red fg green bg\x1b[0m"
            );
            await ctx.value.proxy.writeln(
                "\x1b[32;41m\u{E0B4} green fg red bg\x1b[0m"
            );
            await ctx.value.proxy.writeln(
                "\x1b[31;42;7m\u{E0B4} red fg green bg inverse\x1b[0m"
            );
            await ctx.value.proxy.writeln(
                "\x1b[32;41;7m\u{E0B4} green fg red bg inverse\x1b[0m"
            );
            await ctx.value.proxy.selectAll();
            await pollFor(
                ctx.value.page,
    ...
    test.describe("allowTransparency", async () => {
        test.beforeEach(() =>
            ctx.value.page.evaluate(`term.options.allowTransparency = true`)
        );
    ...
    test.describe("decoration color overrides", async () => {
      ...
      `);
            ...
            await pollFor(
                ...
                () => getCellColor(ctx.value, 1, 1),
                [255, 0, 0, 255]
            ); // inverse foreground of '■' should be decoration fg override
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 2, 1),
                [255, 255, 255, 255]
            ); // inverse background of ' ' should be default foreground
        });
      ...
      `);
        ...
        (ctx.skipCanvasExceptions ? test.skip : test)(
            "backgroundColor should ignore inverse (only bg on decoration)",
            async () => {
                const data = `\x1b[7m■ \x1b[0m`;
                await ctx.value.proxy.write(data);
                await ctx.value.page.evaluate(`
      ...
      `);
                await pollFor(
                    ...
                    () => getCellColor(ctx.value, 1, 1),
                    [0, 0, 0, 255]
                ); // inverse foreground of '■' should be default
                await pollFor(
                    ctx.value.page,
                    () => getCellColor(ctx.value, 2, 1),
                    [0, 0, 255, 255]
                ); // inverse background of ' ' should be decoration bg override
            }
        );
    });
    ...
    test.describe("regression tests", () => {
        (ctx.skipCanvasExceptions ? test.skip : test)(
            "#4736: inactive selection background should replace regular cell background color",
            async () => {
                const theme: ITheme = {
                ...
                await pollFor(
                    ctx.value.page,
                    () => getCellColor(ctx.value, 2, 3),
                    [255, 0, 0, 255]
                );
                await pollFor(
                    ctx.value.page,
                    () => getCellColor(ctx.value, 3, 3),
                    [255, 0, 0, 255]
                );
        ...
        (ctx.skipDomExceptions ? test.skip : test)(
        ...
        test("#4799: cursor should be in the correct position", async () => {
            ...
            await pollFor(
                ctx.value.page,
                () => getCellColor(ctx.value, 1, rows, CellColorPosition.FIRST),
                [0, 0, 255, 255]
            );
        });
    });
 ...
 * treatment.
 */
export function injectSharedRendererTestsStandalone(
    ctx: ISharedRendererTestContext,
    setupCb: () => Promise<void> | void
): void {
...
function getCellColor(
    ...
    col: number,
    row: number,
    position: CellColorPosition = CellColorPosition.CENTER
): MaybeAsync<[red: number, green: number, blue: number, alpha: number]> {
    if (!frameDetails) {
        return getFrameDetails(ctx).then((frameDetails) =>
            getCellColorInner(frameDetails, col, row)
        );
    }
    switch (position) {
...
let frameDetails:
    | { cols: number; rows: number; decoded: IImage32 }
    | undefined = undefined;
async function getFrameDetails(
    ctx: ITestContext
): Promise<{ cols: number; rows: number; decoded: IImage32 }> {
    const screenshotOptions: LocatorScreenshotOptions | undefined = process.env
        .DEBUG
        ? { path: "out-test/playwright/screenshot.png" }
        : undefined;
    const buffer = await ctx.page
...
function getCellColorInner(
    frameDetails: { cols: number; rows: number; decoded: IImage32 },
    col: number,
    row: number
    ...
    const x = Math.floor(
        (col - 1 /* 1- to 0-based index */ + 0.5) /* middle of cell */ *
            cellSize.width
    );
    const y = Math.floor(
        (row - 1 /* 1- to 0-based index */ + 0.5) /* middle of cell */ *
            cellSize.height
    );
    const i =
        (y * frameDetails.decoded.width + x) * 4; /* 4 channels per pixel */
    return Array.from(frameDetails.decoded.data.slice(i, i + 4)) as [
        number,
        number,
        number,
        number,
