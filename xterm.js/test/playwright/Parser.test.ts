...
test.describe("Parser Integration Tests", () => {
    ...
    test.describe("registerCsiHandler", () => {
        test("should call custom CSI handler with js array params", async () => {
            ...
            await ctx.proxy.write(
                "\x1b[38;5;123mparams\x1b[38:2::50:100:150msubparams"
            );
            deepStrictEqual(
        ...
        test("async", async () => {
            ...
            deepStrictEqual(
                await ctx.page.evaluate(() => window.customCsiHandlerCallStack),
                ["C", "B", "A"]
            );
            deepStrictEqual(
                await ctx.page.evaluate(() => window.customCsiHandlerParams),
    ...
    test.describe("registerDcsHandler", () => {
        test("should respects return value", async () => {
            await ctx.proxy.evaluate(([term]) => {
                ...
                window.disposables = [
                    term.parser.registerDcsHandler(
                        ...
                        (data, params) => {
                            window.customDcsHandlerCallStack!.push([
                                "A",
                                params,
                                data,
                            ]);
            ...
            deepStrictEqual(
                await ctx.page.evaluate(() => window.customDcsHandlerCallStack),
                [
                    ["C", [1, 2], "some data"],
                    ["B", [1, 2], "some data"],
                ]
    ...
    test.describe("registerEscHandler", () => {
        test("should respects return value", async () => {
            ...
            deepStrictEqual(
                await ctx.page.evaluate(() => window.customEscHandlerCallStack),
                ["C", "B"]
            );
        });
        test("async", async () => {
    ...
    test.describe("registerOscHandler", () => {
        test("should respects return value", async () => {
            ...
            deepStrictEqual(
                await ctx.page.evaluate(() => window.customOscHandlerCallStack),
                [
                    ["C", "some data"],
                    ["B", "some data"],
                ]
                ...
                window.disposables = [
                    ...
                    term.parser.registerOscHandler(666, (data) => {
                        return new Promise((res) => setTimeout(res, 50)).then(
                            () => {
                                window.customOscHandlerCallStack!.push([
                                    "B",
                                    data,
                                ]);
                                return false;
