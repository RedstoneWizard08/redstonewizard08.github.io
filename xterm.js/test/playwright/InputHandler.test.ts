...
test.describe("InputHandler Integration Tests", () => {
    test.describe("CSI", () => {
      ...
      `);
            await pollFor(ctx.page, () => getLinesAsArray(5), [
                "",
                "b",
                "",
                "a",
      ...
      `);
            await pollFor(ctx.page, () => getLinesAsArray(3), [
                "aoo",
                "",
                "  b",
            ]);
      ...
      `);
            await pollFor(
                ctx.page,
                () => ctx.page.evaluate(`window.term.buffer.active.length`),
                5
            );
            await pollFor(ctx.page, () => getLinesAsArray(5), [
                "   4",
      ...
      `);
        ...
        `);
                ...
                await ctx.page.mouse.move(
                    (coords.left + coords.right) / 2,
                    (coords.top + coords.bottom) / 4
                );
                ok(
                    ((await ctx.page.evaluate(
                        `window.term.getSelection().length`
                    )) as number) > 0,
                    "mouse events are off so there should be a selection"
                );
                ...
                await pollFor(
                    ctx.page,
                    () =>
                        ctx.page.evaluate(`window.term.getSelection().length`),
                    0
                );
                // Enable mouse events
        ...
        `);
                await pollFor(
                    ctx.page,
                    () => ctx.page.evaluate(`window.result`),
                    "\x1b[0n"
                );
            });
        ...
        test.describe.skip(
            "CSI ? Ps n - DSR: Device Status Report (DEC-specific).",
            () => {
                // TODO: Implement
            }
        );
        ...
        test.describe("CSI Ps ; Ps ; Ps t - Window Options", () => {
            ...
            return new Promise((r) => window.term.write('', () => { _h.dispose(); r(); }));
                ...
                await pollFor(
                    ctx.page,
                    async () => await ctx.page.evaluate(`(() => _stack)()`),
                    []
                );
            });
            test("14 - GetWinSizePixels", async () => {
                await ctx.page.evaluate(
                    `window.term.options.windowOptions = { getWinSizePixels: true }; `
                );
                await ctx.page.evaluate(`(() => {
            window._stack = [];
            const _h = window.term.onData(data => window._stack.push(data));
            ...
            return new Promise((r) => window.term.write('', () => { _h.dispose(); r(); }));
                ...
                await pollFor(
                    ctx.page,
                    async () => await ctx.page.evaluate(`(() => _stack)()`),
                    [`\x1b[4;${d.height};${d.width}t`]
                );
            });
            test("16 - GetCellSizePixels", async () => {
                await ctx.page.evaluate(
                    `window.term.options.windowOptions = { getCellSizePixels: true }; `
                );
                await ctx.page.evaluate(`(() => {
            window._stack = [];
            const _h = window.term.onData(data => window._stack.push(data));
            ...
            return new Promise((r) => window.term.write('', () => { _h.dispose(); r(); }));
                ...
                await pollFor(
                    ctx.page,
                    async () => await ctx.page.evaluate(`(() => _stack)()`),
                    [`\x1b[6;${d.cellHeight};${d.cellWidth}t`]
                );
            });
        });
    });
    ...
    test.describe("OSC", () => {
        test.describe("OSC 4", () => {
            test.beforeAll(async () => {
                await ctx.page.evaluate(
                    "(() => {window._recordedData = []; window._h = term.onData(d => window._recordedData.push(d));})()"
                );
            });
            test.afterAll(async () => {
            ...
            test("query single color", async () => {
                ...
                deepStrictEqual(
                    await ctx.page.evaluate("window._recordedData"),
                    ["\x1b]4;0;rgb:2e2e/3434/3636\x1b\\"]
                );
                ...
                deepStrictEqual(
                    ...
                    [
                        "\x1b]4;0;rgb:2e2e/3434/3636\x1b\\",
                        "\x1b]4;77;rgb:5f5f/d7d7/5f5f\x1b\\",
                    ]
                );
            });
            ...
            test("set & query single color", async () => {
                await ctx.proxy.write("\x1b]4;0;?\x07");
                const restore: string[] = await ctx.page.evaluate(
                    "window._recordedData"
                );
                deepStrictEqual(
                    await ctx.page.evaluate("window._recordedData"),
                    restore
                ...
                await ctx.proxy.write(
                    "\x1b]4;0;rgb:01/02/03\x07\x1b]4;0;?\x07"
                );
                deepStrictEqual(
                    await ctx.page.evaluate("window._recordedData"),
                    [restore[0], "\x1b]4;0;rgb:0101/0202/0303\x1b\\"]
                );
                ...
                deepStrictEqual(
                    ...
                    [
                        restore[0],
                        "\x1b]4;0;rgb:0101/0202/0303\x1b\\",
                        restore[0],
                    ]
                );
            });
            test("query & set colors mixed", async () => {
                ...
                await ctx.proxy.write(
                    "\x1b]4;0;rgb:01/02/03;43;?;77;#aabbcc\x07"
                );
                deepStrictEqual(
                    await ctx.page.evaluate("window._recordedData"),
                    ["\x1b]4;43;rgb:0000/d7d7/afaf\x1b\\"]
                );
                await ctx.page.evaluate("window._recordedData.length = 0;");
                ...
                deepStrictEqual(
                    ...
                    [
                        "\x1b]4;0;rgb:0101/0202/0303\x1b\\",
                        "\x1b]4;77;rgb:aaaa/bbbb/cccc\x1b\\",
                    ]
                );
                await ctx.page.evaluate("window._recordedData.length = 0;");
                // restore old values for 0 + 77
                await ctx.proxy.write(
                    restore[0] + restore[1] + "\x1b]4;0;?;77;?\x07"
                );
                deepStrictEqual(
                    await ctx.page.evaluate("window._recordedData"),
                    restore
        ...
        test.describe("OSC 4 & 104", () => {
            ...
            test("change & restore single color", async () => {
                ...
                for (const i of [0, 43, 77, 255]) {
                    ...
                    await ctx.proxy.write(
                        `\x1b]4;${i};rgb:01/02/03\x07\x1b]4;${i};?\x07`
                    );
                    deepStrictEqual(
                        await ctx.page.evaluate("window._recordedData"),
                        [restore[0], `\x1b]4;${i};rgb:0101/0202/0303\x1b\\`]
                    );
                    // restore slot color
                    ...
                    deepStrictEqual(
                        ...
                        [
                            restore[0],
                            `\x1b]4;${i};rgb:0101/0202/0303\x1b\\`,
                            restore[0],
                        ]
                    );
                    await ctx.page.evaluate("window._recordedData.length = 0;");
            ...
            test("restore multiple at once", async () => {
                ...
                await ctx.proxy.write(
                    `\x1b]104;0;43;77\x07` + `\x1b]4;0;?;43;?;77;?\x07`
                );
                deepStrictEqual(
                    await ctx.page.evaluate("window._recordedData"),
                    restore
...
async function getLinesAsArray(
    count: number,
    start: number = 0
): Promise<string[]> {
    let text = "";
...
async function getDimensions(): Promise<any> {
    const dim: IRenderDimensions = await ctx.page.evaluate(
