...
test.describe("API Integration Tests", () => {
    ...
    test("Proposed API check", async () => {
        await openTerminal(
            ...
            { loadUnicodeGraphemesAddon: false }
        );
    ...
    `);
        ...
        await pollFor(
            ctx.page,
            `window.term.buffer.active.getLine(1).translateToString(true)`,
            "bar"
        );
        await pollFor(
            ctx.page,
            `window.term.buffer.active.getLine(2).translateToString(true)`,
            "文"
        );
    });
    ...
    test.describe("options", () => {
        test("getter", async () => {
            await openTerminal(ctx);
            strictEqual(
                await ctx.page.evaluate(`window.term.options.cols`),
                80
            );
            strictEqual(
                await ctx.page.evaluate(`window.term.options.rows`),
                24
            );
        });
        test("setter", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(`window.term.options.scrollback`),
                1
            );
            await ctx.page.evaluate(`
        window.term.options = {
      ...
      `);
            strictEqual(
                await ctx.page.evaluate(`window.term.options.fontSize`),
                30
            );
            strictEqual(
                await ctx.page.evaluate(`window.term.options.fontFamily`),
                "Arial"
            );
        });
        test("object.keys return the correct number of options", async () => {
            ...
            notStrictEqual(
                await ctx.page.evaluate(
                    `Object.keys(window.term.options).length`
                ),
                0
            );
        });
    ...
    test.describe("renderer", () => {
        test("foreground", async () => {
            ...
            await pollFor(
                ctx.page,
                `document.querySelectorAll('.xterm-rows > :nth-child(1) > *').length`,
                9
            );
            deepStrictEqual(
                await ctx.page.evaluate(`
      ...
      `),
        ...
        test("background", async () => {
            await openTerminal(ctx);
            await ctx.proxy.write(
                "\x1b[40m0\x1b[41m1\x1b[42m2\x1b[43m3\x1b[44m4\x1b[45m5\x1b[46m6\x1b[47m7"
            );
            await pollFor(
                ctx.page,
                `document.querySelectorAll('.xterm-rows > :nth-child(1) > *').length`,
    ...
    test("selection", async () => {
        ...
        await ctx.proxy.write(`\n\nfoo\n\n\rbar\n\n\rbaz`);
        strictEqual(
            await ctx.page.evaluate(`window.term.hasSelection()`),
            false
        );
        strictEqual(await ctx.page.evaluate(`window.term.getSelection()`), "");
        deepStrictEqual(
            await ctx.page.evaluate(`window.term.getSelectionPosition()`),
            undefined
        );
        ...
        if (process.platform === "win32") {
            strictEqual(
                await ctx.page.evaluate(`window.term.getSelection()`),
                "\r\n\r\nfoo\r\n\r\nbar\r\n\r\nbaz"
            );
        } else {
            strictEqual(
    ...
    test("focus, blur", async () => {
        await openTerminal(ctx);
        strictEqual(
            await ctx.page.evaluate(`document.activeElement.className`),
            ""
        );
    ...
    test.describe("buffer", () => {
        test("cursorX, cursorY", async () => {
            await openTerminal(ctx, { rows: 5, cols: 5 });
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.active.cursorX`),
                0
            );
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.active.cursorY`),
                0
            );
        ...
        test("viewportY", async () => {
            await openTerminal(ctx, { rows: 5 });
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.active.viewportY`),
                0
            );
            await ctx.proxy.write("\n\n\n\n");
            strictEqual(
        ...
        test("baseY", async () => {
            await openTerminal(ctx, { rows: 5 });
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.active.baseY`),
                0
            );
            await ctx.proxy.write("\n\n\n\n");
            strictEqual(
        ...
        test("length", async () => {
            await openTerminal(ctx, { rows: 5 });
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.active.length`),
                5
            );
            await ctx.proxy.write("\n\n\n\n");
            strictEqual(
        ...
        test.describe("getLine", () => {
            test("invalid index", async () => {
                ...
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(-1)`
                    ),
                    undefined
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(5)`
                    ),
                    undefined
                );
            });
            ...
            test("isWrapped", async () => {
                ...
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).isWrapped`
                    ),
                    false
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(1).isWrapped`
                    ),
                    false
                );
                await ctx.proxy.write("abcde");
            ...
            test("translateToString", async () => {
                ...
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).translateToString()`
                    ),
                    "     "
                );
                strictEqual(
                ...
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).translateToString(false, 1)`
                    ),
                    "ooba"
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).translateToString(false, 1, 3)`
                    ),
                    "oo"
                );
            });
            ...
            test("getCell", async () => {
                ...
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(-1)`
                    ),
                    undefined
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(5)`
                    ),
                    undefined
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(0).getChars()`
                    ),
                    ""
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(0).getWidth()`
                    ),
                    1
                );
                ...
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(1).getChars()`
                    ),
                    "文"
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(1).getWidth()`
                    ),
                    2
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(2).getChars()`
                    ),
                    ""
                );
                strictEqual(
                    await ctx.page.evaluate(
                        `window.term.buffer.active.getLine(0).getCell(2).getWidth()`
                    ),
                    0
                );
            });
          ...
          for (let i = 0; i < window.term.markers.length; ++i) {
              ...
              marker.onDispose(() => window.disposeStack.push(marker));
                ...
                await ctx.page.evaluate(`window.term.clear()`);
                strictEqual(
                    await ctx.page.evaluate(`window.disposeStack.length`),
                    4
                );
            });
        });
        ...
        test("active, normal, alternate", async () => {
            await openTerminal(ctx, { cols: 5 });
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.active.type`),
                "normal"
            );
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.normal.type`),
                "normal"
            );
            strictEqual(
                await ctx.page.evaluate(`window.term.buffer.alternate.type`),
                "alternate"
            );
            ...
            strictEqual(
                await ctx.page.evaluate(
                    `window.term.buffer.normal.getLine(0).translateToString()`
                ),
                "norm "
            );
            strictEqual(
                await ctx.page.evaluate(
                    `window.term.buffer.alternate.getLine(0)`
                ),
                undefined
            );
            ...
            strictEqual(
                await ctx.page.evaluate(
                    `window.term.buffer.alternate.getLine(0).translateToString()`
                ),
                "alt  "
            );
    ...
    test.describe("modes", () => {
        ...
        test("applicationCursorKeysMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(
                    `window.term.modes.applicationCursorKeysMode`
                ),
                true
            );
        ...
        test("applicationKeypadMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(
                    `window.term.modes.applicationKeypadMode`
                ),
                true
            );
        ...
        test("bracketedPasteMode", async () => {
            await ctx.proxy.write("\x1b[?2004h");
            strictEqual(
                await ctx.page.evaluate(`window.term.modes.bracketedPasteMode`),
                true
            );
        ...
        test("insertMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(`window.term.modes.insertMode`),
                true
            );
        ...
        test("mouseTrackingMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(`window.term.modes.mouseTrackingMode`),
                "x10"
            );
        ...
        test("originMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(`window.term.modes.originMode`),
                true
            );
        ...
        test("reverseWraparoundMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(
                    `window.term.modes.reverseWraparoundMode`
                ),
                true
            );
        ...
        test("sendFocusMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(`window.term.modes.sendFocusMode`),
                true
            );
        ...
        test("wraparoundMode", async () => {
            ...
            strictEqual(
                await ctx.page.evaluate(`window.term.modes.wraparoundMode`),
                true
            );
    ...
    `);
        strictEqual(
            await ctx.page.evaluate(`window.term._core._isDisposed`),
            true
        );
    });
    ...
    `);
        ...
        await pollFor(
            ctx.page,
            `window.term._core._renderService.dimensions.css.cell.width > 0`,
            true
        );
    });
    ...
    test.describe("registerDecoration", () => {
        test.describe("bufferDecorations", () => {
            test("should register decorations and render them when terminal open is called", async () => {
                await openTerminal(ctx);
                await ctx.page.evaluate(
                    `window.marker1 = window.term.registerMarker(1)`
                );
                await ctx.page.evaluate(
                    `window.marker2 = window.term.registerMarker(2)`
                );
                await ctx.page.evaluate(
                    `window.term.registerDecoration({ marker: window.marker1 })`
                );
                await ctx.page.evaluate(
                    `window.term.registerDecoration({ marker: window.marker2 })`
                );
                await pollFor(
                    ctx.page,
            ...
            test("should return undefined when the marker has already been disposed of", async () => {
                await openTerminal(ctx);
                await ctx.page.evaluate(
                    `window.marker = window.term.registerMarker(1)`
                );
                await ctx.page.evaluate(`window.marker.dispose()`);
                await pollFor(
                    ctx.page,
                    `window.decoration = window.term.registerDecoration({ marker: window.marker });`,
                    undefined
                );
            });
      ...
      `);
        ...
        test.describe("overviewRulerDecorations", () => {
            test("should not add an overview ruler when width is not set", async () => {
                ...
                await ctx.page.evaluate(
                    `window.term.registerDecoration({ marker: window.marker1, overviewRulerOptions: { color: 'red', position: 'full' } })`
                );
                await ctx.page.evaluate(
                    `window.term.registerDecoration({ marker: window.marker2, overviewRulerOptions: { color: 'blue', position: 'full' } })`
                );
                await pollFor(
                    ctx.page,
                    `document.querySelectorAll('.xterm-decoration-overview-ruler').length`,
                    0
                );
            });
...
async function getCellCoordinates(
    ...
    return {
        x:
            dimensions.left +
            dimensions.renderDimensions.device.cell.width * (col - 0.5),
        y:
            dimensions.top +
            dimensions.renderDimensions.device.cell.height * (row - 0.5),
    };
}

async function moveMouseCell(
    dimensions: IDimensions,
    col: number,
    row: number
): Promise<void> {
