...
export async function createTestContext(
    ...
    page.on("console", (e) =>
        console.log(`[${browser.browserType().name()}:${e.type()}]`, e)
    );
    page.on("pageerror", (e) =>
        console.error(`[${browser.browserType().name()}]`, e)
    );
 ...
 */
type PlaywrightApiProxy<
    TBaseInterface,
    TAsyncPropOverrides extends keyof TBaseInterface,
    TAsyncMethodOverrides extends keyof TBaseInterface,
    TCustomOverrides extends keyof TBaseInterface,
> =
    ...
    EnsureAsyncProperties<Pick<TBaseInterface, TAsyncPropOverrides>> &
        ...
        Omit<
            TBaseInterface,
            TAsyncPropOverrides | TAsyncMethodOverrides | TCustomOverrides
        >;

interface ITerminalProxyCustomMethods {
    evaluate<T>(
        pageFunction: PageFunction<JSHandle<Terminal>[], T>
    ): Promise<T>;
    write(data: string | Uint8Array): Promise<void>;
}

type TerminalProxyAsyncPropOverrides = "cols" | "rows" | "modes";
type TerminalProxyAsyncMethodOverrides =
    | "hasSelection"
    | "getSelection"
    | "getSelectionPosition"
    | "registerMarker"
    | "registerDecoration";
type TerminalProxyCustomOverrides =
    | "buffer"
...
export class TerminalProxy
    implements
        ITerminalProxyCustomMethods,
        PlaywrightApiProxy<
            Terminal,
            TerminalProxyAsyncPropOverrides,
            TerminalProxyAsyncMethodOverrides,
            TerminalProxyCustomOverrides
        >
{
    ...
    public async initPage(): Promise<void> {
        ...
        await this._page.exposeFunction("onBinary", (e: string) =>
            this._onBinary.fire(e)
        );
        await this._page.exposeFunction("onCursorMove", () =>
            this._onCursorMove.fire()
        );
        await this._page.exposeFunction("onData", (e: string) =>
            this._onData.fire(e)
        );
        await this._page.exposeFunction(
            "onKey",
            (e: { key: string; domEvent: KeyboardEvent }) => this._onKey.fire(e)
        );
        await this._page.exposeFunction("onLineFeed", () =>
            this._onLineFeed.fire()
        );
        await this._page.exposeFunction(
            "onRender",
            (e: { start: number; end: number }) => this._onRender.fire(e)
        );
        await this._page.exposeFunction(
            "onResize",
            (e: { cols: number; rows: number }) => this._onResize.fire(e)
        );
        await this._page.exposeFunction("onScroll", (e: number) =>
            this._onScroll.fire(e)
        );
        await this._page.exposeFunction("onSelectionChange", () =>
            this._onSelectionChange.fire()
        );
        await this._page.exposeFunction("onTitleChange", (e: string) =>
            this._onTitleChange.fire(e)
        );
        await this._page.exposeFunction("onWriteParsed", () =>
    ...
    public async initTerm(): Promise<void> {
        ...
        await this.evaluate(([term]) =>
            term.onBinary((window as any).onBinary)
        );
        await this.evaluate(([term]) =>
            term.onCursorMove((window as any).onCursorMove)
        );
        await this.evaluate(([term]) => term.onData((window as any).onData));
        await this.evaluate(([term]) => term.onKey((window as any).onKey));
        await this.evaluate(([term]) =>
            term.onLineFeed((window as any).onLineFeed)
        );
        await this.evaluate(([term]) =>
            term.onRender((window as any).onRender)
        );
        await this.evaluate(([term]) =>
            term.onResize((window as any).onResize)
        );
        await this.evaluate(([term]) =>
            term.onScroll((window as any).onScroll)
        );
        await this.evaluate(([term]) =>
            term.onSelectionChange((window as any).onSelectionChange)
        );
        await this.evaluate(([term]) =>
            term.onTitleChange((window as any).onTitleChange)
        );
        await this.evaluate(([term]) =>
            term.onWriteParsed((window as any).onWriteParsed)
        );
    }
    ...
    private _onKey = new EventEmitter<{
        key: string;
    ...
    public async selectLines(start: number, end: number): Promise<void> {
        return this._page.evaluate(
            ([term, start, end]) => term.selectLines(start, end),
            [await this.getHandle(), start, end] as const
        );
    }
    ...
    public async select(
        ...
        return this._page.evaluate(
            ([term, column, row, length]) => term.select(column, row, length),
            [await this.getHandle(), column, row, length] as const
        );
    }
    public async paste(data: string): Promise<void> {
        return this._page.evaluate(([term, data]) => term.paste(data), [
    ...
    public async refresh(start: number, end: number): Promise<void> {
        return this._page.evaluate(
            ([term, start, end]) => term.refresh(start, end),
            [await this.getHandle(), start, end] as const
        );
    }
    public async getOption<T extends keyof ITerminalOptions>(
        key: T
    ): Promise<ITerminalOptions[T]> {
        return this._page.evaluate(([term, key]) => term.options[key as T], [
            await this.getHandle(),
            key,
        ] as const);
    }
    public async setOption<T extends keyof ITerminalOptions>(
        key: T,
        value: ITerminalOptions[T]
    ): Promise<any> {
        return this._page.evaluate(
            ([term, key, value]) =>
                (term.options[key as T] = value as ITerminalOptions[T]),
            [await this.getHandle(), key, value] as const
        );
    }
    ...
    public async scrollPages(pageCount: number): Promise<void> {
        return this._page.evaluate(
            ([term, pageCount]) => term.scrollPages(pageCount),
            [await this.getHandle(), pageCount] as const
        );
    }
    public async scrollToLine(line: number): Promise<void> {
        return this._page.evaluate(([term, line]) => term.scrollToLine(line), [
            await this.getHandle(),
            line,
        ] as const);
    }
    public async scrollLines(amount: number): Promise<void> {
        return this._page.evaluate(
            ([term, amount]) => term.scrollLines(amount),
            [await this.getHandle(), amount] as const
        );
    }
    ...
    public async input(
        data: string,
        wasUserInput: boolean = true
    ): Promise<void> {
        return this.evaluate(([term]) => term.input(data, wasUserInput));
    }
    public async resize(cols: number, rows: number): Promise<void> {
        return this._page.evaluate(
            ([term, cols, rows]) => term.resize(cols, rows),
            [await this.getHandle(), cols, rows] as const
        );
    }
    public async registerMarker(y?: number | undefined): Promise<IMarker> {
        return this._page.evaluate(([term, y]) => term.registerMarker(y), [
            await this.getHandle(),
            y,
        ] as const);
    }
    public async registerDecoration(
        decorationOptions: IDecorationOptions
    ): Promise<IDecoration | undefined> {
        return this._page.evaluate(
            ([term, decorationOptions]) =>
                term.registerDecoration(decorationOptions),
            [await this.getHandle(), decorationOptions] as const
        );
    }
    ...
    public async evaluateHandle<T>(
        ...
        return this._page.evaluateHandle(pageFunction, [
            await this.getHandle(),
        ]);
    }
        ...
        >
{
    ...
    public get active(): TerminalBufferProxy {
        return new TerminalBufferProxy(
            this._page,
            this._proxy,
            this._proxy.evaluateHandle(([term]) => term.buffer.active)
        );
    }
    public get normal(): TerminalBufferProxy {
        return new TerminalBufferProxy(
            this._page,
            this._proxy,
            this._proxy.evaluateHandle(([term]) => term.buffer.normal)
        );
    }
    public get alternate(): TerminalBufferProxy {
        return new TerminalBufferProxy(
            this._page,
            this._proxy,
            this._proxy.evaluateHandle(([term]) => term.buffer.alternate)
        );
    }
}
...
class TerminalBufferProxy /* implements EnsureAsyncProperties<IBuffer>*/ {
    ...
    public async getLine(y: number): Promise<TerminalBufferLine | undefined> {
        const lineHandle = await this._page.evaluateHandle(
            ([buffer, y]) => buffer.getLine(y),
            [await this._handle, y] as const
        );
        const value = await lineHandle.jsonValue();
        if (value) {
            return new TerminalBufferLine(
                this._page,
                lineHandle as JSHandle<IBufferLine>
            );
        }
        return undefined;
    }
    ...
    public async evaluate<T>(
        pageFunction: PageFunction<JSHandle<IBuffer>[], T>
    ): Promise<T> {
...
class TerminalBufferLine {
    ...
    public translateToString(
        ...
        return this._page.evaluate(
            ([bufferLine, trimRight, startColumn, endColumn]) => {
    ...
    public async getCell(x: number): Promise<TerminalBufferCell | undefined> {
        const cellHandle = await this._page.evaluateHandle(
            ([bufferLine, x]) => bufferLine.getCell(x),
            [this._handle, x] as const
        );
        const value = await cellHandle.jsonValue();
        if (value) {
            return new TerminalBufferCell(
                this._page,
                cellHandle as JSHandle<IBufferCell>
            );
        }
        return undefined;
    }
    ...
    public async evaluate<T>(
        pageFunction: PageFunction<JSHandle<IBufferLine>[], T>
    ): Promise<T> {
...
class TerminalBufferCell {
    ...
    public async evaluate<T>(
        pageFunction: PageFunction<JSHandle<IBufferCell>[], T>
    ): Promise<T> {
        return this._page.evaluate(pageFunction, [this._handle]);
    }
}
...
class TerminalCoreProxy {
    ...
    public get renderDimensions(): Promise<IRenderDimensions> {
        return this.evaluate(
            ([core]) =>
                ((core as any)._renderService as IRenderService).dimensions
        );
    }

    public async triggerBinaryEvent(data: string): Promise<void> {
        return this._page.evaluate(
            ([core, data]) => core.coreService.triggerBinaryEvent(data),
            [await this._getCoreHandle(), data] as const
        );
    }

    private async _getCoreHandle(): Promise<JSHandle<ICoreTerminal>> {
        return this._proxy.evaluateHandle(
            ([term]) => (term as any)._core as ICoreTerminal
        );
    }

    public async evaluate<T>(
        pageFunction: PageFunction<JSHandle<ICoreTerminal>[], T>
    ): Promise<T> {
...
export async function openTerminal(
    ...
    testOptions: { loadUnicodeGraphemesAddon: boolean } = {
        loadUnicodeGraphemesAddon: true,
    }
): Promise<void> {
  ...
  `);
    ...
    strictEqual(
        await ctx.page.evaluate(
            `document.querySelector('#terminal-container').children.length`
        ),
        0,
        "there must be no terminals on the page"
    );
    await ctx.page.evaluate(`
...
export async function pollFor<T>(
    ...
    evalOrFn: string | (() => MaybeAsync<T>),
    val: T,
    preFn?: () => Promise<void>,
    options?: IPollForOptions<T>
): Promise<void> {
    if (!options) {
        options = {};
    }
    ...
    const result =
        typeof evalOrFn === "string"
            ? await page.evaluate(evalOrFn)
            : await evalOrFn();
    ...
    if (!equalityCheck) {
        ...
        return new Promise<void>((r) => {
            setTimeout(
                () =>
                    r(
                        pollFor(page, evalOrFn, val, preFn, {
                            ...options,
...
export async function pollForApproximate<T>(
    page: playwright.Page,
...
export function getBrowserType():
    | playwright.BrowserType<playwright.WebKitBrowser>
    | playwright.BrowserType<playwright.ChromiumBrowser>
    | playwright.BrowserType<playwright.FirefoxBrowser> {
    // Default to chromium
    let browserType:
        | playwright.BrowserType<playwright.WebKitBrowser>
        | playwright.BrowserType<playwright.ChromiumBrowser>
    ...
    if (
        index !== -1 &&
        process.argv.length > index + 1 &&
        typeof process.argv[index + 1] === "string"
    ) {
