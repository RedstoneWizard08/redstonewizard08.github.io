...
import { type Font, loadBuffer } from "font-ligatures";
...
let fontsPromise: Promise<Record<string, IFontMetadata[]>> | undefined =
    undefined;
...
export default async function load(
    ...
    if (!fontsPromise) {
        ...
        if (typeof navigator !== "undefined" && "fonts" in navigator) {
            try {
                const status = await (
                    navigator as unknown as IFontAccessNavigator
                ).permissions.request?.({
                    name: "local-fonts",
                });
