...
export default class ScopedEval {
    ...
    build(code: string, stringInterpolationMode = false): (scope: any) => any {
        return new Function(
            ...this.preprocess(code, stringInterpolationMode)
        ) as (scope: any) => any;
    }

    preprocess(
        code: string,
        stringInterpolationMode = false
    ): [string, string] {
        if (typeof code !== "string") {
            throw new Error(
                `Code to be evaluated must be a string, but received ${typeof code}: ${JSON.stringify(code)}`
            );
        }
        ...
        if (count > 0) {
            ...
            if (lastStatement.type === "ExpressionStatement") {
                m.insert(lastStatement.range?.[0] ?? 0, "return ");
            }
        }
