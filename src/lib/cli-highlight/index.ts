...
import { DEFAULT_THEME, plain, type Theme } from "./theme";

function colorizeNode(
    node: domhandler.ChildNode,
    theme: Theme = {},
    context?: string,
): string {
    switch (node.type) {
        case "text": {
            const text = (node as domhandler.Text).data;
        ...
        case "tag": {
            const hljsClass = /hljs-(\w+)/.exec(
                (node as domhandler.Element).attribs.class,
            );
            
            if (hljsClass) {
                const token = hljsClass[1];
                
                const nodeData = (node as domhandler.Element).childNodes
                    .map((node) => colorizeNode(node, theme, token))
                    .join("");

                return ((theme as any)[token] ||
                    (DEFAULT_THEME as any)[token] || plain)(nodeData);
            }
            ...
            return (node as domhandler.Element).childNodes.map((node) =>
                colorizeNode(node, theme)
            ).join("");
        }
    }
...
function colorize(code: string, theme: Theme = {}): string {
    const fragment = parse5.parseFragment<Htmlparser2TreeAdapterMap>(code, {
